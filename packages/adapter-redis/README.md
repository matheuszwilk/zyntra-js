...
[![NPM Version](https://img.shields.io/npm/v/@zyntra-js/adapter-redis.svg)](https://www.npmjs.com/package/@zyntra-js/adapter-redis)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

The official Redis adapter for the **ZyntraJS Store** system. This package provides a high-performance driver for caching and pub/sub messaging using a Redis data store.

## Role in the Ecosystem

This adapter acts as the concrete implementation for the abstract Store interface defined in `@zyntra-js/core`. By plugging this adapter into your application, you enable powerful features:
...
-   **Shared Connection:** The Redis client from this adapter can be shared with other systems, like the `ZyntraJS Queues` (BullMQ) adapter, for maximum efficiency.
...
export const store = createRedisStoreAdapter({
  ...
  keyPrefix: 'zyntra-app:',
});
```

### 2. Register with the Zyntra Builder

Next, enable the Store feature in your main `zyntra.ts` file by passing your `store` adapter instance to the `.store()` method on the builder.
...
Your application is now configured to use Redis for caching and pub/sub. You can access the store's methods via `zyntra.store` or `context.store` within your actions.
