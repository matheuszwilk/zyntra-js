# AI Agent Maintenance Manual: `@zyntra-js/core`

**Version:** 1.0.0
**For AI Agent:** You are an expert TypeScript software engineer. This document is your primary technical guide to the `@zyntra-js/core` package. Read and understand it thoroughly before attempting any modifications. Your goal is to perform maintenance tasks accurately, respecting the architectural principles outlined here.
...
### 1.2. Purpose
This package is the heart of the ZyntraJS framework. It contains all the essential, non-adapter-specific logic for building, configuring, and running a type-safe API. It provides the foundational building blocks, including the main builder, the request processing pipeline, all core TypeScript interfaces, and the client-side hooks. This package is a required dependency for any application built with ZyntraJS.
...
### 2.1. The Builder Pattern: `ZyntraBuilder`

The entire application is constructed using a fluent (chainable) builder API, starting with the `Zyntra` object exported from `builder.service.ts`.

*   **Why it's used:** This pattern enforces a structured and guided setup process. It allows the framework to build up its type system dynamically. As you chain methods like `.store(redisAdapter)` or `.jobs(bullmqAdapter)`, the builder not only registers the functionality but also injects the corresponding types into the global `zyntra` instance and the request `context`.
*   **The Flow:**
    1.  `Zyntra.context<T>()`: **Always the first call.** It sets the base type for the application's global context.
    2.  `.logger()`, `.store()`, `.jobs()`, `.plugins()`: These methods attach modules and extend the framework's capabilities and types.
    3.  `.create()`: **Always the final call.** It consumes the entire configuration and returns a fully-typed, immutable `zyntra` instance. This instance holds the factory functions (`.query()`, `.mutation()`, `.controller()`, etc.) that you use to build your API.
...
2.  **Route Resolution (`RouteResolverProcessor`)**: The processor first matches the incoming request's method (`GET`, `POST`, etc.) and path (`/users/123`) to a specific `ZyntraAction` that you defined in a controller. If no route is found, a `404 Not Found` response is returned immediately.
...
3.  **Context Building (`ContextBuilderProcessor`)**: Once a route is matched, the `ContextBuilderProcessor` creates the initial `ProcessedContext`. This involves:
    ...
    *   Creating the base application context by calling your `createZyntraAppContext` function.
...
5.  **Action Handler Execution**: The `handler` function of the matched `ZyntraAction` is finally executed. It receives the fully-enriched `ZyntraActionContext`, which contains the request details, the extended application context, response helpers, and plugin accessors.

6.  **Response Processing (`ResponseProcessor`)**: The value returned by your action handler is processed. If you return a plain object, it's converted into a standard JSON `Response`. If you use the `response` helpers (e.g., `response.success()`, `response.unauthorized()`), the `ZyntraResponseProcessor` constructs the appropriate HTTP `Response` with the correct status code and headers.
...
The **Context** is the sole dependency injection (DI) mechanism in ZyntraJS.
...
*   `src/index.ts`
    > **Purpose**: The public entry point of the `@zyntra-js/core` package. It exports the primary `Zyntra` builder, the `createZyntraPlugin` factories, and all essential types that developers need to interact with the framework.
...
*   `src/services/`
    ...
    *   `builder.service.ts`: Contains the `ZyntraBuilder` class. **This is the most important file in this directory.** It is the starting point of every application. Any new chainable method on the `Zyntra` object (like a future `.cache()` method) would be added here.
    *   `action.service.ts`: Exports `createZyntraQuery` and `createZyntraMutation`. These factories produce the `ZyntraAction` objects that define your API endpoints.
    *   `controller.service.ts`: Exports `createZyntraController`. A very simple factory that structures and groups a collection of actions under a common path.
    *   `procedure.service.ts`: Exports `createZyntraProcedure` and the enhanced builder/factories. This is where the logic for creating middleware resides.
    *   `router.service.ts`: Exports `createZyntraRouter`. This factory takes the final configuration (controllers, context, plugins) and produces the `AppRouter`, which is used by the `RequestProcessor`.
    *   `cookie.service.ts`: Contains the `ZyntraCookie` class, a helper for parsing and serializing request/response cookies in a standardized way.
    *   `jobs.service.ts`: Defines the **structure** and **abstractions** for the ZyntraJS Queues system (e.g., `JobsRegistry`, `JobsRouter`). The concrete implementation is provided by an adapter like `@zyntra-js/adapter-bullmq`.
    *   `plugin.service.ts`: Contains the `ZyntraPluginManager`. This is a sophisticated service that handles plugin registration, dependency resolution, lifecycle hook execution, and event bus management.
    *   `realtime.service.ts`: Implements the `ZyntraRealtimeService`, providing the high-level API (`.publish()`, `.to()`, `.broadcast()`) for interacting with the SSE system.
...
*   `src/processors/`
    ...
    *   `sse.processor.ts`: The low-level engine for managing Server-Sent Events (SSE) connections and channels. `ZyntraRealtimeService` is a high-level API over this processor.
...
*   `src/types/`
    ...
    *   `action.interface.ts`: Defines `ZyntraAction`, `ZyntraActionContext`, `ZyntraQueryOptions`, `InferEndpoint`, etc. **This is arguably the most important type definition file.** It defines the shape of the `ctx` object passed to every handler.
    *   `client.interface.ts`: Defines all types related to the client-side hooks (`useQuery`, `useMutation`, `useRealtime`).
    *   `controller.interface.ts`: Defines `ZyntraControllerConfig`.
    *   `procedure.interface.ts`: Defines `ZyntraProcedure` and the types for the enhanced builder.
    *   `jobs.interface.ts`: Defines the canonical shapes for job queue adapters, routers, and job definitions (`ZyntraJobQueueAdapter`, `JobsRouter`, `JobDefinition`).
    *   `store.interface.ts`: Defines the `ZyntraStoreAdapter` interface, which is the contract for all store adapters (e.g., Redis).
    *   `plugin.interface.ts`: Defines the comprehensive `ZyntraPlugin` interface and its related types.
    *   (and others...)

*   `src/client/`
    > **Purpose**: Contains code that is specifically intended for the client-side (frontend) portion of the ZyntraJS ecosystem.
    *   `zyntra.hooks.ts`: The React hook implementations for `useQuery`, `useMutation`, and `useRealtime`. This file is marked with `"use client";`.

*   `src/error/`
    > **Purpose**: Defines custom error classes for the framework.
    *   `index.ts`: Contains the `ZyntraError` class, used for throwing structured, standardized errors within the framework.
...
*   `src/utils/`
    ...
    *   `logger.ts`: Contains centralized logging utilities (`resolveLogLevel`, `createLoggerContext`) that provide consistent logging behavior across all processors and services. These utilities handle environment variable resolution (`ZYNTRA_LOG_LEVEL`) with proper fallbacks and context creation for structured logging.

*   `src/adapters/`
    > **Purpose**: Contains built-in adapter implementations that don't require external dependencies.
    *   `telemetry.console.ts`: The default console-based telemetry adapter that implements `ZyntraTelemetryProvider`. Uses the centralized logging system for consistent output formatting.
...
The ZyntraJS core implements a centralized logging system designed for consistency, maintainability, and AI-friendly debugging. All logging throughout the framework follows these principles:

*   **Single Source of Truth**: All logging configuration is controlled by the `ZYNTRA_LOG_LEVEL` environment variable
...
- Returns a context object that can be passed to `ZyntraConsoleLogger`
...
**Example Pattern:**
```typescript
const logger = new ZyntraConsoleLogger(
  resolveLogLevel(),
...
- ❌ Mixing `NODE_ENV` with logging levels - Use only `ZYNTRA_LOG_LEVEL`
...
The request processing pipeline is the heart of ZyntraJS. Understanding the flow and responsibilities of each processor is crucial for maintenance and debugging.
...
- **Error Types**: Handles ZyntraError, validation errors, generic errors
...
Services in ZyntraJS are factory functions and classes that create and configure framework components. They are the "builder" layer that developers interact with.

#### Service Categories:

**Core Builder Services**:
- `builder.service.ts`: The main ZyntraBuilder class with fluent API
...
ZYNTRA_LOG_LEVEL=DEBUG

# Enable trace logging for maximum verbosity
ZYNTRA_LOG_LEVEL=TRACE

# Enable debug logging only for specific components (if implemented)
ZYNTRA_LOG_LEVEL=DEBUG
ZYNTRA_DEBUG_COMPONENTS=RequestProcessor,RouteResolver
```

#### Common Debugging Scenarios:

**Request Pipeline Issues**:
1. Set `ZYNTRA_LOG_LEVEL=DEBUG` to see request flow
...
**Performance Issues**:
1. Set `ZYNTRA_LOG_LEVEL=TRACE` for detailed timing information
...
1. Enable debug logging: `ZYNTRA_LOG_LEVEL=DEBUG`
...
// Error handling
"[ErrorHandler] Handling ZyntraError: Validation failed"
```
...
// Use ZYNTRA_LOG_LEVEL=DEBUG in tests to verify logging
process.env.ZYNTRA_LOG_LEVEL = 'DEBUG';
```
...
```typescript
import { ZyntraConsoleLogger } from '../utils/logger';
import { resolveLogLevel, createLoggerContext } from '../utils/logger';

export class ExampleProcessor {
  private readonly logger: ZyntraConsoleLogger;

  constructor() {
    this.logger = new ZyntraConsoleLogger(
      resolveLogLevel(),
...
```typescript
export class ZyntraProcessorError extends ZyntraError {
  constructor(
    message: string,
    ...
    this.name = 'ZyntraProcessorError';
  }
}
```
...
try {
  ...
  const processorError = new ZyntraProcessorError(
...
describe('ExampleProcessor', () => {
  ...
  let mockLogger: jest.Mocked<ZyntraConsoleLogger>;
...
- **Configurability**: Single environment variable (`ZYNTRA_LOG_LEVEL`) controls entire system
...
const logger = new ZyntraConsoleLogger(logLevel, createLoggerContext('Component'));
```
...
This section provides step-by-step guides for performing common maintenance and enhancement tasks on the `@zyntra-js/core` package.

### Task 1: Add a New Option to `zyntra.query()`
**Scenario:** We want to add a new top-level option to all query actions, for example, `cacheTTL: number`.

1.  **Objective:** Modify the `createZyntraQuery` factory to accept and process a new option.
2.  **Locate Type Definition:** Open `packages/core/src/types/action.interface.ts`.
3.  **Modify Interface:** Add the new optional property to the `ZyntraQueryOptions` type definition.
    ```typescript
    // In ZyntraQueryOptions
    export type ZyntraQueryOptions<...> = {
      // ... existing options
      cacheTTL?: number; // Add the new option here
    };
    ```
4.  **Locate Implementation:** Open `packages/core/src/services/action.service.ts`.
5.  **Update Factory Function:** In the `createZyntraQuery` function, access the new option from the `options` object and include it in the returned action object.
    ```typescript
    // In createZyntraQuery
    return {
      ...options,
      method: 'GET' as const,
...
6.  **Update Processor:** The logic that *uses* this option would likely reside in the `RequestProcessor`. Open `packages/core/src/processors/request.processor.ts`. In the `executeAction` or `handleSuccessfulResponse` method, you would add logic like:
    ...
    ```
7.  **Write/Update Tests:** Navigate to `packages/core/src/services/__tests__/action.service.test.ts` and add a new test case to verify that the `cacheTTL` option is correctly passed through by the `createZyntraQuery` factory. If applicable, add tests to the `request.processor.test.ts` to verify the new caching behavior.
...
2.  **Define Hook Type:** Open `packages/core/src/types/request.processor.ts` (or a more appropriate location like a new `hooks.interface.ts`). Define the function signature for the new hook.
    ...
    ```
3.  **Update Router/Builder Configuration:** The hook needs to be configured. Decide where it should be passed. A logical place is in the main `zyntra.router()` configuration. Open `packages/core/src/types/router.interface.ts` and add the hook to `ZyntraRouterConfig`.
    ```typescript
    // In ZyntraRouterConfig
    export interface ZyntraRouterConfig<...> {
      // ... existing properties
...
### Task 3: Modify the `ZyntraCookie` Service
**Scenario:** We need to add a new method, `.toJSON()`, to the `ZyntraCookie` class that returns all cookies as a plain object.

1.  **Objective:** Add a new public method to the `ZyntraCookie` service.
2.  **Locate Service:** Open `packages/core/src/services/cookie.service.ts`.
3.  **Add New Method:** Add the `toJSON` method to the `ZyntraCookie` class.
    ```typescript
    // In ZyntraCookie class
    /**
...
2. **Update Logger Interface:** Extend `ZyntraConsoleLogger` if needed
...
export function createStructuredLogger(component: string, metadata?: Record<string, any>) {
  ...
  return new ZyntraConsoleLogger(resolveLogLevel(), {
    ...baseContext,
    ...metadata
  });
}
...
The ZyntraJS framework has evolved significantly, particularly in its logging and architecture patterns. Key maturity indicators include:
