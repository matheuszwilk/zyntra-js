...
import type { ZyntraAction, ZyntraControllerConfig, ZyntraRouter, InferRouterCaller, ClientConfig } from '../types';
import { parseURL } from '../utils/url';
import { createUseQuery, createUseMutation, createUseRealtime } from './zyntra.hooks';

/**
 * Creates a browser-side client for Zyntra Router
 ...
 */
export function createBrowserClient<TRouter extends ZyntraRouter<any, any, any, any, any>>(
  router: TRouter
  ...
  for (const controllerName in router.controllers) {
    ...
    const controller = router.controllers[controllerName] as ZyntraControllerConfig<any>;
    const parsedBaseURL = parseURL(baseURL, basePATH, controller.path);

    for (const actionName in controller.actions) {
      const action = controller.actions[actionName] as ZyntraAction<any, any, any, any, any, any, any, any, any, any>;
 ...
 */
function createActionFetcher<TAction extends ZyntraAction<any, any, any, any, any, any, any, any, any, any>>(
  action: TAction,
  baseURL: string,
) {
  return async (options?: TAction['$Infer']['$Input']): Promise<TAction['$Infer']['$Response']> => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        throw new Error(`ZyntraClient fetch error: ${error.message}`);
      }
      throw error;
    }
  };
