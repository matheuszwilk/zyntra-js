...
import { ZyntraConsoleLogger } from "../services";
import { ZyntraLogLevel, type ZyntraRouter } from "../types";
import type {
  ZyntraContextType,
  RefetchFn,
...
/**
 * Zyntra context provider type
 */
const ZyntraContext = createContext<ZyntraContextType<any> | undefined>(
  undefined,
);

/**
 * Options for the Zyntra Provider
 */
export interface ZyntraProviderOptions<TContext extends () => Promise<any> | any> {
  /**
  ...
  /**
   * Get the context for the ZyntraProvider
   */
  getContext?: TContext;

  /**
   * The context to be passed to the ZyntraProvider
   */
  getScopes?: (ctx: Awaited<ReturnType<TContext>>) => Promise<string[]> | string[];
}

/**
 * Provider component for the Zyntra context, managing query invalidation and refetching.
 *
 * @component
 * @param {PropsWithChildren} props - The component props
 * @param {React.ReactNode} props.children - Child components to be wrapped by the provider
 * @param {ZyntraProviderOptions} props.options - Configuration options
 *
 * @example
 * ```tsx
 * <ZyntraProvider options={{ enableRevalidation: true }}>
 *   <App />
 * </ZyntraProvider>
 * ```
 *
 * @remarks
 * The ZyntraProvider manages a collection of query listeners and provides methods to:
 ...
 *
 * @returns {JSX.Element} Provider component wrapping its children with Zyntra context
 */
export function ZyntraProvider<TContext extends () => Promise<any> | any>({
  children,
  getContext,
  getScopes,
  debug,
  ...
  reconnectDelay = 1000,
}: PropsWithChildren & ZyntraProviderOptions<TContext>) {
  ...
  const logger = useMemo(() => {
    return ZyntraConsoleLogger.create({
      level: debug ? ZyntraLogLevel.DEBUG : ZyntraLogLevel.INFO,
      context: {
        provider: 'ZyntraProvider',
        package: 'core'
      }
    });
  }, []);
  ...
  return (
    <ZyntraContext.Provider value={contextValue}>
      {children}
    </ZyntraContext.Provider>
  );
}

/**
 * Hook to access the Zyntra context, providing access to query registration and invalidation methods.
 *
 * @returns {ZyntraContextType} Zyntra context object
 *
 * @throws {ZyntraError} Throws an error if the hook is used outside of an ZyntraProvider
 */
export const useZyntraQueryClient = <
  TRouter extends Omit<ZyntraRouter<any, any, any, any, any>, 'caller'> & { caller: any }
>() => {
  const context = useContext(ZyntraContext) as
    | ZyntraContextType<TRouter>
    | undefined;

  if (!context) {
    throw new Error(
      "useZyntraQueryClient must be used within an ZyntraProvider",
    );
  }
