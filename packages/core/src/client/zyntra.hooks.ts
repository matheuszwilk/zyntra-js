...
import { useZyntraQueryClient } from "./zyntra.context";
...
type InferZyntraResponse<T> = T extends { data: infer TData, error: infer TError } ? { data: TData | null, error: TError | null } : { data: null, error: null };
...
>() => {
  return () => {
    const { register, unregister, invalidate } = useZyntraQueryClient();
    ...
    const retry = useCallback(() => {
      if (lastUsedParamsRef.current) {
        ...
        const logger = ZyntraConsoleLogger.create({
          ...
          context: createLoggerContext('ZyntraHooks')
        });
...
export function useRealtime<T = any>(
  ...
  const { subscribeToRealtime } = useZyntraQueryClient();
 ...
 */
export const createUseRealtime = <TAction extends ZyntraAction<any, any, any, any, any, any, any, any, any, any>>(
  controller: string,
  action: string,
) => {
  return (options?: RealtimeActionCallerOptions<TAction>): RealtimeActionCallerResult<TAction> => {
    ...
    // @ts-expect-error - Ignore type error for now
    const [response, setResponse] = useState<InferZyntraResponse<Awaited<TAction["$Infer"]["$Output"]>>>(() => ({
      data: options?.initialData?.data || null,
      error: options?.initialData?.error || null,
    }));
