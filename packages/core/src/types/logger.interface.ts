/**
 * Log levels for the Zyntra Logger provider.
 * These levels are used to categorize log messages by severity.
 */
export enum ZyntraLogLevel {
  FATAL = 'fatal',
  ERROR = 'error',
  WARN = 'warn',
  INFO = 'info',
 ...
 */
export interface ZyntraLogEntry {
 ...
 */
export interface ZyntraLoggerOptions {
  level?: ZyntraLogLevel;
  /**
   ...
   */
  formatter?: (entry: ZyntraLogEntry) => string;
  /**
...
/**
 * Interface for a robust, extensible logger provider for the Zyntra framework.
 * 
...
export interface ZyntraLogger {
   ...
   */
  log(level: ZyntraLogLevel, message: string, context?: any, error?: Error | unknown): void;
