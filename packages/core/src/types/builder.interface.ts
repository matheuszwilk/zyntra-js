...
/**
 * Base configuration interface for the Zyntra Framework.
 * Defines the base configuration options that can be applied to an Zyntra application instance.
 */
export type ZyntraBaseConfig = {
  baseURL?: string;
  basePATH?: string;
}
...
/**
 * Configuration interface for the Zyntra Framework builder setup.
 * Defines all the core adapters and configuration options that can be applied
 * to an Zyntra application instance.
 *
 ...
 * ```typescript
 * // Basic configuration
 * type BasicConfig = ZyntraBuilderConfig<
 *   { db: Database },
 *   RedisStoreAdapter,
 *   ConsoleLogger,
 *   BullMQAdapter,
 ...
 * type CallbackConfig = ZyntraBuilderConfig<
 *   (req: Request) => Promise<{ user: User; db: Database }>,
 *   ZyntraStoreAdapter,
 *   ZyntraLogger,
 *   JobsNamespaceProxy<any>,
 *   ZyntraTelemetryProvider
 * >;
 * ```
 */
export interface ZyntraBuilderConfig<
  TContext extends object | ContextCallback,
  TConfig extends ZyntraBaseConfig,
  TStore extends ZyntraStoreAdapter = any,
  TLogger extends ZyntraLogger = any,
  TJobs extends JobsNamespaceProxy<TContext> = any,
  TTelemetry extends ZyntraTelemetryProvider = any,
  TPlugins extends Record<string, ZyntraPlugin<any, any, any, any, any, any, any, any>> = Record<string, any>,
...
> {
   ...
   * ```
   */
  middleware?: readonly ZyntraProcedure<unknown, unknown, unknown>[];
   ...
   * ```
   */
  store: TStore extends ZyntraStoreAdapter ? TStore : never;
   ...
   * ```
   */
  logger: TLogger extends ZyntraLogger ? TLogger : never;
   ...
   * ```
   */
  telemetry: TTelemetry extends ZyntraTelemetryProvider ? TTelemetry : never;
   ...
   */
  realtime: ZyntraRealtimeService
 ...
 * @example
 * ```typescript
 * const withAuth: ZyntraBuilderExtension<AppContext> = (builder) =>
 *   builder.middleware([authProcedure({ required: true })]);
 *
 * const withRateLimit: ZyntraBuilderExtension<AppContext> = (builder) =>
 ...
 * ```
 */
export type ZyntraBuilderExtension<
  TContext extends object, 
  TConfig extends ZyntraBaseConfig, 
  TMiddlewares extends readonly ZyntraProcedure<unknown, unknown, unknown>[], 
  ...
  TRealtime extends ZyntraRealtimeService, 
  TPlugins extends Record<string, ZyntraPlugin<any, any, any, any, any, any, any, any>>, 
  TDocs extends DocsConfig
> = (
  builder: ZyntraBuilder<
    TContext, 
    TConfig, 
    TStore, 
    TLogger, 
 ...
 * Ensures all plugins implement the ZyntraPlugin interface correctly.
 *
 * @template TPlugins - Record of plugin name to plugin instance
 *
 * @example
