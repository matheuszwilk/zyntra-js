import type { HTTPMethod, ZyntraCookies, ZyntraHeaders } from "./action.interface";
...
import { ZyntraResponseProcessor } from "@/processors";

/**
 * Represents the context for an Zyntra procedure.
 * @template TActionContext - The type of the custom action context.
 *
 * @typedef {Object} ZyntraProcedureContext
 ...
 * @property {ZyntraHeaders} request.headers - The request headers.
 * @property {ZyntraCookies} request.cookies - The request cookies.
 * @property {TActionContext} context - Custom context passed to the procedure.
 * @property {ZyntraResponseProcessor} response - The response processor for handling the procedure output.
 *
 * @since 1.0.0
 */
export type ZyntraProcedureContext<TActionContext> = {
  request: {
    path: string;
    params: any;
    body: any;
  ...
  response: ZyntraResponseProcessor<TActionContext>;
  /**
...
/**
 * Represents a procedure in the Zyntra framework.
 ...
 */
export type ZyntraProcedure<
  TActionContext,
  TOptions,
  TOutput
> = {
  name: string;
  handler: (options: TOptions, ctx: ZyntraProcedureContext<TActionContext>) => Promise<TOutput> | TOutput;
}
 ...
 * ```typescript
 * // Basic usage
 * const loggerProcedure = zyntra.procedure.simple(async (ctx) => {
 *   ctx.context.logger.info('Request received', { path: ctx.request.path });
 *   return { requestId: crypto.randomUUID() };
 * });
 *
 * // With typed options
 * const authProcedure = zyntra.procedure()
...
export interface EnhancedProcedureBuilder<TActionContext> {
   ...
   * @example
   * ```typescript
   * const procedure = zyntra.procedure()
   ...
   * @example
   * ```typescript
   * const loggerProcedure = zyntra.procedure()
  ...
  ): ZyntraProcedure<TActionContext, undefined, TOutput>;
}
...
export interface EnhancedProcedureBuilderWithName<TActionContext, TName extends string> {
  ...
  handler<TOutput>(
    handler: EnhancedProcedureHandler<TActionContext, undefined, TOutput>
  ): ZyntraProcedure<TActionContext, undefined, TOutput> & { name: TName };
}
 ...
 * const apiEndpoint = zyntra.query({
...
export interface EnhancedProcedureFactories<TActionContext> {
   ...
   * @example
   * ```typescript
   * const requestLogger = zyntra.procedure.simple(async ({ request, context }) => {
   ...
   * @example
   * ```typescript
   * const authProcedure = zyntra.procedure.withSchema({
   *   optionsSchema: z.object({
   *     required: z.boolean().default(false),
   *     roles: z.array(z.string()).optional(),
   ...
   * const adminAction = zyntra.query({
   ...
   * @example
   * ```typescript
   * const rateLimitProcedure = zyntra.procedure.fromConfig({
   *   name: 'rate-limiter',
   *   optionsSchema: z.object({
   *     max: z.number().min(1).max(1000).default(100),
   *     windowMs: z.number().min(1000).default(60000),
 ...
 */
type ExtractProcedureOutput<T> = T extends ZyntraProcedure<any, any, infer TOutput> ? TOutput : never;

/**
 * Infers the procedure context type from an array of ZyntraProcedure instances.
 *
 * @typeParam TActionProcedures - An array of ZyntraProcedure instances with generic types for input, output, and context
 * @returns A union-to-intersection type of the output types of all procedures
 *
 * @example
 * ```typescript
 * type Procedures = [
 *   ZyntraProcedure<any, any, { user: User }>,
 *   ZyntraProcedure<any, any, { requestId: string }>
 * ]
 * type Context = InferProcedureContext<Procedures> // { user: User } & { requestId: string }
 * ```
 */
export type InferProcedureContext<TActionProcedures extends readonly ZyntraProcedure<unknown, unknown, unknown>[]> =
  TActionProcedures extends readonly []
    ? {}
    : UnionToIntersection<
        TActionProcedures extends readonly (infer TProcedure)[]
          ? TProcedure extends ZyntraProcedure<any, any, infer TOutput>
...
/**
 * Infers the context type from an array of ZyntraProcedure instances.
 */
export type InferActionProcedureContext<
  TActionProcedures extends readonly ZyntraProcedure<any, any, unknown>[] | undefined
> =
