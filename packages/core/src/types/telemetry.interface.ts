/**
 * Telemetry interfaces for the Zyntra Router framework.
 ...
 */
export interface ZyntraTelemetrySpan {
  /** Nome do span */
  ...
  finish(): void;
  /** Criar child span */
  child(name: string, options?: ZyntraSpanOptions): ZyntraTelemetrySpan;
  /** Obter contexto do span para propagação */
  getContext(): ZyntraSpanContext;
}

/**
 * Contexto de span para propagação entre serviços.
 */
export interface ZyntraSpanContext {
  traceId: string;
  spanId: string;
  traceFlags: number;
 ...
 */
export interface ZyntraTelemetryProvider {
  /** Configuração do provider */
  readonly config: ZyntraTelemetryConfig;
  /** Nome do provider */
  readonly name: string;
  /** Status do provider */
  ...
  /** Iniciar novo span */
  startSpan(name: string, options?: ZyntraSpanOptions): ZyntraTelemetrySpan;
  /** Obter span ativo atual */
  getActiveSpan(): ZyntraTelemetrySpan | null;
  /** Definir span como ativo */
  setActiveSpan(span: ZyntraTelemetrySpan): void;
  /** Executar função com span ativo */
  withActiveSpan<T>(span: ZyntraTelemetrySpan, fn: () => T): T;
  ...
  /** Timer (duração) */
  timer(metric: string, tags?: Record<string, string>): ZyntraTimer;
 ...
 */
export interface ZyntraTelemetryConfig {
  /** Nome do serviço */
  serviceName: string;
  /** Versão do serviço */
 ...
 */
export interface ZyntraSpanOptions {
  /** Span pai */
  parent?: ZyntraTelemetrySpan;
  /** Contexto de span pai para propagação */
  parentContext?: ZyntraSpanContext;
  /** Tags iniciais */
 ...
 */
export interface ZyntraContextWithTelemetry<TContext extends object> {
  /** Provider de telemetria */
  telemetry?: ZyntraTelemetryProvider;
  /** Span ativo atual */
  span?: ZyntraTelemetrySpan;
  /** Contexto de trace para propagação */
  traceContext?: ZyntraSpanContext;
}
...
export type TelemetryProviderFactory<TConfig = any> = (
  config: ZyntraTelemetryConfig & TConfig
) => ZyntraTelemetryProvider | Promise<ZyntraTelemetryProvider>;

/**
 * Utility types para inferência de tipos.
 */
export type InferTelemetryProvider<T> = 
  T extends ZyntraTelemetryProvider ? T : never;

export type InferTelemetryConfig<T> = 
  T extends ZyntraTelemetryProvider ? T['config'] : never;
