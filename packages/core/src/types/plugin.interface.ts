import type { ContextCallback, ZyntraProcedure } from "../types";
...
import { ZyntraResponseProcessor } from "@/processors/response.processor";
...
> = {
  ...
  handler: (params: {
    ...
    response: ZyntraResponseProcessor<TContext>;
  }) => any | Promise<any>;
};
 ...
 */
export function createZyntraPluginAction<
  TContext extends object | ContextCallback,
  TName extends string,
  TInput extends StandardSchemaV1,
  TOutput,
 ...
 */
export function createZyntraPluginEventListener<
 ...
 */
export function createZyntraPluginEventEmitter<
  const TEventName extends string,
  TPayload extends StandardSchemaV1,
>(config: {
  event: TEventName;
...
> =
  {
    global?: ZyntraProcedure<TContext, any, any>[];
    routes?: Record<RoutePattern, ZyntraProcedure<TContext, any, any>[]>;
  };
 ...
 * @example
 * ```typescript
 * const authPlugin = createZyntraPlugin({
 *   name: 'auth',
 *   $actions: {
 *     validateToken: createZyntraPluginAction({ ... }),
 *     createSession: createZyntraPluginAction({ ... })
 *   },
 *   $controllers: (actions) => ({
 *     login: {
 *       handler: async ({ self }) => {
 ...
 * ```
 */
export function createZyntraPlugin<
  TContext extends object | ContextCallback,
...
export type InferPluginActions<T> =
  T extends ZyntraPlugin<any, any, any, any, any, infer TActions, any, any>
    ? TActions
    : never;
...
export type InferPluginContext<T> =
  T extends ZyntraPlugin<infer TContext, any, any, any, any, any, any, any>
    ? TContext
    : never;
...
export type InferPluginContextExtensions<TPlugins extends Record<string, any>> =
  {
    [K in keyof TPlugins]: TPlugins[K] extends ZyntraPlugin<
      any,
      any,
      any,
      any,
 ...
 * ```
 */
export type InferZyntraPlugins<TPlugins extends Record<string, any>> =
