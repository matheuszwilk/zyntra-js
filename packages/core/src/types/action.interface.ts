import type { ZyntraResponseProcessor } from "../processors/response.processor";
import type { ZyntraCommonErrorCode, ZyntraResponse } from "./response.interface";
import type { ZyntraCookie } from "../services/cookie.service";
import type { ZyntraProcedure, InferActionProcedureContext } from "./procedure.interface";
...
import type { ZyntraRealtimeService } from "./realtime.interface";
import type { ZyntraPlugin, InferZyntraPlugins } from "./plugin.interface";
...
export type ZyntraCookies = ZyntraCookie;
export type ZyntraHeaders = Headers;

/**
 * Infers the caller signature for a given ZyntraAction.
 ...
 export type InferActionResponse<TReturn> =
   TReturn extends any
     ? TReturn extends Promise<infer TData>
       ...
       : TReturn extends ZyntraResponse<infer TDataType, infer TErrorType>
         ? ZyntraResponse<TDataType, TErrorType>
...
type InferWebResponse<T> = {
  data?: unknown;
  error?: ZyntraErrorResponse<ZyntraCommonErrorCode>;
};

/**
 * Infer types when returning value directly
 */
type InferDirectResponse<T> = T extends ZyntraResponse<infer TData, infer TError>
  ? ZyntraResponse<TData, TError>
  : {
      data?: T;
      error?: ZyntraErrorResponse;
    };

/**
 * Base error response type
 */
type ZyntraErrorResponse<
  TCode = ZyntraCommonErrorCode,
  TData = unknown
> = {
  code: TCode;
  message?: string;
...
type InferActionContext<
  ...
  TActionMiddlewares extends readonly ZyntraProcedure<any, any, unknown>[] | undefined
> = TActionContext & (
  TActionMiddlewares extends readonly ZyntraProcedure<any, any, unknown>[]
 ...
 * ```typescript
 * // In action handler
 * handler: async (ctx: ZyntraActionContext<AppContext, "/users/:id", "POST", UserSchema, never, []>) => {
 ...
 * ```
 */
export type ZyntraActionContext<
  TActionContext extends object,
  ...
  TActionMiddlewares extends readonly ZyntraProcedure<any, unknown, unknown>[] | undefined,
...
> = {
  ...
  request: {
    ...
    headers: ZyntraHeaders;
    cookies: ZyntraCookies;
   ...
   */
   response: ZyntraResponseProcessor<InferActionContext<TActionContext, TActionMiddlewares>>;

  /**
   * Realtime service for server-sent events and websocket communication.
   * Enables real-time updates to connected clients.
   */
  realtime: ZyntraRealtimeService<InferActionContext<TActionContext, TActionMiddlewares>>;
   ...
   * ```
   */
  plugins: InferZyntraPlugins<TActionPlugins>
};

export type ZyntraActionHandler<
  TActionContext extends ZyntraActionContext<any, any, any, any, any, any, any>,
  TActionResponse,
...
export type ZyntraQueryOptions<
  ...
  TQueryMiddlewares extends readonly ZyntraProcedure<any, any, unknown>[] | undefined,
...
export type ZyntraMutationOptions<
  ...
  TMutationMiddlewares extends readonly ZyntraProcedure<any, any, unknown>[] | undefined,
...
export type ZyntraAction<
  ...
  TActionQuery extends StandardSchemaV1 | undefined,
  TActionMiddlewares extends readonly ZyntraProcedure<any, any, unknown>[] | undefined,
  TActionPlugins extends Record<string, ZyntraPlugin<any, any, any, any, any, any, any, any>>,
  TActionHandler extends ZyntraActionHandler<
    ZyntraActionContext<
      TActionContext,
      TActionPath,
      TActionMethod,
...
> = Prettify<{
  ...
  $Errors: TActionInferResponse extends { error: infer TError } ? TError : ZyntraErrorResponse<ZyntraCommonErrorCode>;
