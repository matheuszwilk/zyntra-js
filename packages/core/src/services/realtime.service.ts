...
/**
 * Type-safe, fluent RealtimeBuilder implementation for Zyntra.js.
 *
...
class RealtimeBuilderImpl<TContext = unknown> implements RealtimeBuilder<TContext> {
   ...
   */
  constructor(store: ZyntraStoreAdapter, initial: RealtimeEventPayload) {
    this.store = store;
...
/**
 * ZyntraRealtimeService provides a type-safe, developer-friendly API for realtime event publishing.
 *
 * @typeParam TContext - The context type available to event scopes.
 *
 * @example
 * // Create the service (usually injected via builder)
 * const realtime = new ZyntraRealtimeService(store);
 *
 ...
 */
export class ZyntraRealtimeService<TContext = any>
  implements ZyntraRealtimeServiceType<TContext>
{
  private readonly store: ZyntraStoreAdapter;

  /**
   * Construct a new ZyntraRealtimeService.
   *
   * @param store - The store adapter for event publishing (reserved for future use).
   *
   * @example
   * const realtime = new ZyntraRealtimeService(store);
   */
  constructor(store: ZyntraStoreAdapter) {
    this.store = store;
  }
