...
import type { ContextCallback, ZyntraBaseContext } from "../types/context.interface";
...
/**
 * 100% Type-Safe Plugin Manager for Zyntra.js with StoreAdapter Integration
 *
 ...
 * @example
 * ```typescript
 * const pluginManager = new ZyntraPluginManager<MyContext>({
 ...
 * ```
 */
export class ZyntraPluginManager<
...
> {
  ...
  private readonly store: ZyntraStoreAdapter;
  private readonly logger: ZyntraLogger;
   ...
   *
   * Events are published to Redis channels with pattern: `zyntra:plugin:events:{eventName}`
  ...
  async emit<TPayload>(
    ...
    try {
      ...
      const eventChannel = `zyntra:plugin:events:${eventName}`;
  ...
  private createPluginProxy<TActions extends PluginActionsCollection<any>>(
    plugin: ZyntraPlugin<any, any, any, any, any, any, any, any>,
  ): void {
