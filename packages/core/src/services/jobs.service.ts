 ...
 */
export interface ZyntraJobsServiceConfig<TContext extends object> {
  /** The job queue adapter to use (e.g., BullMQ) */
  adapter: ZyntraJobQueueAdapter<TContext>;
 ...
 * 
 * This service acts as the primary interface for job management in Zyntra applications,
...
> {
  ...
  bulkRegister<TNewJobs extends Record<string, JobDefinition<TContext, any, any>>>(
    jobs: TNewJobs
  ): ZyntraJobsService<TContext, TJobs & TNewJobs> {
    ...
    const newService = new ZyntraJobsService<TContext, TJobs & TNewJobs>({
 ...
 * 
 * const jobsService = createZyntraJobsService({
 *   adapter: createBullMQAdapter({ store: redisStore }),
 *   contextFactory: () => ({ db, logger, email })
 * });
 * ```
 */
export function createZyntraJobsService<TContext extends object>(
  config: ZyntraJobsServiceConfig<TContext>
): ZyntraJobsService<TContext> {
  return new ZyntraJobsService(config);
}
