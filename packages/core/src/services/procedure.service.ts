...
/**
 * Creates a reusable middleware procedure for the Zyntra Framework.
 ...
 * const authProcedure = createZyntraProcedure({
 *   name: 'auth',
 ...
 * const protectedAction = createZyntraQuery({
 *   path: 'protected',
 *   use: [authProcedure()],
 *   handler: (ctx) => {
 ...
 */
export const createZyntraProcedure = <
  TActionContext = unknown,
  TOptions = unknown,
  TOutput = unknown
>(Procedure: ZyntraProcedure<TActionContext, TOptions, TOutput>) => (options?: TOptions): ZyntraProcedure<TActionContext, TOptions, TOutput> => {
  return {
    ...Procedure,
    // @ts-expect-error - This is a hack to get around the circular dependency
    handler: (ctx: ZyntraProcedureContext<TActionContext>) => Procedure.handler(options, ctx)
  }
};
 ...
 * This is the recommended way to create procedures in modern Zyntra applications.
 * 
 ...
 * const timestampProcedure = zyntra.procedure()
 *   .name('timestamp')
    ...
    name<TName extends string>(name: TName): EnhancedProcedureBuilderWithName<TActionContext, TName> {
      return {
        options<TOptionsSchema extends StandardSchemaV1>(
          ...
          return {
            ...
            > & { name: TName } {
              return {
                name,
                handler: (options: StandardSchemaV1.InferInput<TOptionsSchema>, ctx: ZyntraProcedureContext<TActionContext>) => {
              ...
              } as ZyntraProcedure<TActionContext, StandardSchemaV1.InferInput<TOptionsSchema>, TOutput> & { name: TName };
            }
          };
        },
        ...
        handler<TOutput>(
          ...
          return {
            name,
            handler: (_options: undefined, ctx: ZyntraProcedureContext<TActionContext>) => {
          ...
          } as ZyntraProcedure<TActionContext, undefined, TOutput> & { name: TName };
        }
      };
    },
      ...
      return {
        ...
        handler: (
          options: TOptionsSchema extends StandardSchemaV1 
            ? StandardSchemaV1.InferInput<TOptionsSchema>
            : undefined, 
          ctx: ZyntraProcedureContext<TActionContext>
        ) => {
          // Validate options using the schema
          const validatedOptions = (config.optionsSchema as any).parse ? (config.optionsSchema as any).parse(options) : options;
