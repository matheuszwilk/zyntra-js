...
import type { ContextCallback, DocsConfig, ZyntraBaseConfig, ZyntraControllerBaseAction, ZyntraControllerConfig, ZyntraRouter, InferServerRouterCaller, MutationActionCallerResult, QueryActionCallerResult, RealtimeActionCallerResult } from "../types";
...
>(
  ...
  processor: RequestProcessor<ZyntraRouter<TContext, TControllers, TConfig, TPlugins, TDocs>>
): InferServerRouterCaller<ZyntraRouter<TContext, TControllers, TConfig, TPlugins, TDocs>> {
  const caller = new Proxy({} as InferServerRouterCaller<ZyntraRouter<TContext, TControllers, TConfig, TPlugins, TDocs>>, {
    get(_, controllerName: string) {
      ...
      return new Proxy({}, {
        get(_, actionName: string) {
          const action = controller.actions[actionName] as ZyntraControllerBaseAction;
          if (!action) {
