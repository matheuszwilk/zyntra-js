import type { ZyntraRouter } from "../types";
...
/**
 * Lista MÍNIMA de pacotes específicos do Zyntra.js (server-only)
 */
const ZYNTRA_PACKAGES = [
...
const PACKAGE_CONFIG = {
  server: [
    // Apenas adapters do Zyntra que são definitivamente server-only
    ...ZYNTRA_PACKAGES,
  ],
  client: [
    // Apenas Node.js built-ins que sempre causam problema
    ...NODE_MODULES,
    // E alguns adapters específicos do Zyntra
    ...ZYNTRA_PACKAGES,
  ],
} as const;
 ...
 */
const safeLog = (() => {
  let logger: ReturnType<typeof ZyntraConsoleLogger.create> | null = null;
  
  const getLogger = () => {
    if (!logger) {
      try {
        logger = ZyntraConsoleLogger.create({
...
const createEmptyModule = () => {
  ...
  const showWarning = (prop) => {
    if (!warningShown.has(prop) && typeof console !== 'undefined' && console.warn) {
      console.warn(\`[Zyntra] Attempted to use server-only module "\${prop}" in client bundle. This is likely a bug.\`);
...
/**
 * Adapter function to convert an ZyntraRouter instance into Next.js route handlers
 *
 * @param router - An instance of ZyntraRouter that will handle the incoming requests
 * @returns An object containing HTTP method handlers compatible with Next.js route handlers
 * @example
 * ```typescript
 * const router = new ZyntraRouter()
 ...
 */
export const nextRouteHandlerAdapter = (router: Omit<ZyntraRouter<any, any, any, any, any>, 'caller'>) => {
  return {
    GET: (request: Request) => {
...
export const withZyntra = (...configs: NextConfig[]): NextConfig => {
  try {
