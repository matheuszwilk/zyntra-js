...
import { type ZyntraLogger, type ZyntraRouter } from "../types";
import { ZyntraConsoleLogger } from "../services/logger.service";
import type { ZyntraPluginManager } from "../services/plugin.service";
...
/**
 * Context builder processor for the Zyntra Framework.
...
export class ContextBuilderProcessor {
   ...
   */
  static async build<TRouter extends ZyntraRouter<any, any, any, any, any>>(
    ...
    const cookies = new ZyntraCookie(request.headers);
    const response = new ZyntraResponseProcessor();
  ...
  static async enhanceWithPlugins(
    ...
    pluginManager?: ZyntraPluginManager<any>
  ): Promise<ProcessedContext> {
