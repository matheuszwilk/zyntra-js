import { type ZyntraLogger } from "../types"
import { SSEProcessor } from "./sse.processor"
import { ZyntraConsoleLogger } from "../services/logger.service"
import type { CookieOptions } from "../types/cookie.interface"
import { ZyntraCommonErrorCode, ZyntraResponseError, ZyntraResponse } from "../types/response.interface"
...
/**
 * A builder class for creating and manipulating HTTP responses in the Zyntra Framework.
 ...
 * const response = ZyntraResponseProcessor.init<MyContext>()
 *   .status(200)
 *   .setCookie('session', 'abc123', { httpOnly: true })
 *   .success({ user: { id: 1, name: 'John' } }) // Now typed as ZyntraResponseProcessor<MyContext, { user: { id: number, name: string } }>
 *   .toResponse();
 * ```
 */
export class ZyntraResponseProcessor<TContext = unknown> {
  private _status: number = 200
  private _statusExplicitlySet: boolean = false
  private _response = {} as ZyntraResponse
  ...
  private _store?: ZyntraStoreAdapter
  private _context?: TContext
  private _logger?: ZyntraLogger

  private get logger(): ZyntraLogger {
    if (!this._logger) {
      this._logger = ZyntraConsoleLogger.create({
  ...
  /**
   * Creates a new instance of ZyntraResponseProcessor.
   ...
   * @returns A new ZyntraResponseProcessor instance
   *
   * @example
   * ```typescript
   * const response = ZyntraResponseProcessor.init<MyContext>(store, context);
   * ```
   */
  static init<TContext = unknown>(
    store?: ZyntraStoreAdapter,
    context?: TContext
  ): ZyntraResponseProcessor<TContext> {
    const instance = new ZyntraResponseProcessor<TContext>();
   ...
   * @private
   */
  private withData<TNewData>(): ZyntraResponseProcessor<TContext> {
    const newInstance = new ZyntraResponseProcessor<TContext>();
    newInstance._status = this._status;
    newInstance._statusExplicitlySet = this._statusExplicitlySet;
    newInstance._response = {} as ZyntraResponse<TNewData>;
  ...
  stream<TStreamData>(options: StreamOptions<TStreamData>) {
    ...
    const newInstance = this.withData<ZyntraResponse<TStreamData>>();
   ...
   * response.success(user); // Returns ZyntraResponseProcessor<TContext, typeof user>
   * ```
   */
  success<TSuccessData>(data?: TSuccessData) {
    const instance = this.withData<ZyntraResponse<TSuccessData>>()
    instance._response = {} as ZyntraResponse<TSuccessData>;
    ...
    return instance as unknown as ZyntraResponse<TSuccessData>;
  }
   ...
   * response.created(newUser); // Returns ZyntraResponseProcessor<TContext, typeof newUser>
   * ```
   */
  created<TCreatedData>(data: TCreatedData) {
    const instance = this.withData<ZyntraResponse<TCreatedData>>()
    instance._response = {} as ZyntraResponse<TCreatedData>;
    instance._response.data = data as TCreatedData;
    instance._response.error = null;
    if (!this._statusExplicitlySet) instance._status = 201;
    return instance as unknown as ZyntraResponse<TCreatedData>;
  }
   ...
   */
  noContent() {
    const instance = this.withData<ZyntraResponse<null>>()
    instance._response = {} as ZyntraResponse<null>;
    ...
    return instance as unknown as ZyntraResponse<null>;
  }
  ...
  error<TErrorCode extends ZyntraCommonErrorCode>(error: ZyntraResponseError<TErrorCode>) {
    this._status = 400
    this._response = {} as ZyntraResponse<null, ZyntraResponseError<TErrorCode>>;
    ...
    return this as unknown as ZyntraResponse<null, ZyntraResponseError<TErrorCode>>;
  }
  ...
  badRequest<TBadRequestData>(message = 'Bad Request', data?: TBadRequestData) {
    this._response = {} as ZyntraResponse<null, ZyntraResponseError<'ERR_BAD_REQUEST'>>;
    this._response.data = null;
    this._response.error = new ZyntraResponseError({
      message,
      data,
      code: 'ERR_BAD_REQUEST'
    });
    if (!this._statusExplicitlySet) this._status = 400;
    return this as unknown as ZyntraResponse<null, ZyntraResponseError<'ERR_BAD_REQUEST'>>;
  }
  ...
  unauthorized<TUnauthorizedData>(message = 'Unauthorized', data?: TUnauthorizedData) {
    this._response = {} as ZyntraResponse<null, ZyntraResponseError<'ERR_UNAUTHORIZED'>>;
    this._response.data = null;
    this._response.error = new ZyntraResponseError({
      message,
      data,
    ...
    return this as unknown as ZyntraResponse<null, ZyntraResponseError<'ERR_UNAUTHORIZED'>>;
  }
  ...
  forbidden<TForbiddenData>(message = 'Forbidden', data?: TForbiddenData) {
    this._response = {} as ZyntraResponse<null, ZyntraResponseError<'ERR_FORBIDDEN'>>;
    this._response.data = null;
    this._response.error = new ZyntraResponseError({
      message,
      data,
    ...
    return this as unknown as ZyntraResponse<null, ZyntraResponseError<'ERR_FORBIDDEN'>>;
  }
  ...
  notFound<TNotFoundData>(message = 'Not Found', data?: TNotFoundData) {
    this._response = {} as ZyntraResponse<null, ZyntraResponseError<'ERR_NOT_FOUND'>>;
    this._response.data = null;
    this._response.error = new ZyntraResponseError({
      message,
      data,
    ...
    return this as unknown as ZyntraResponse<null, ZyntraResponseError<'ERR_NOT_FOUND'>>;
  }
  ...
  redirect(destination: string, type: 'replace' | 'push' = 'replace') {
    this._response = {} as ZyntraResponse<null, ZyntraResponseError<'ERR_REDIRECT'>>;
    this._response.data = null;
    this._response.error = new ZyntraResponseError({
      message: 'Redirect',
    ...
    return this as unknown as ZyntraResponse<null, ZyntraResponseError<'ERR_REDIRECT'>>;
  }
   ...
   * response.json(result); // Returns ZyntraResponseProcessor<TContext, typeof result>
   * ```
   */
  json<TJsonData>(data: TJsonData) {
    const instance = this.withData<ZyntraResponse<TJsonData>>()
    instance._response = {} as ZyntraResponse<TJsonData>;
    instance._response.data = data as TJsonData;
    instance._response.error = null;
    if (!this._statusExplicitlySet) instance._status = 200;
    return instance as unknown as ZyntraResponse<TJsonData>;
  }
  ...
  private createStream(): Response {
    ...
    if (!SSEProcessor.channelExists(channelId)) {
      ...
      SSEProcessor.registerChannel({
        id: channelId,
        description: `Dynamic channel created by ZyntraResponseProcessor`
      });
    }
