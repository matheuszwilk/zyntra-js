...
import { ZyntraLogLevel, type ZyntraLogger, type ZyntraProcedure } from "../types";
import type { ProcessedContext } from "./context-builder.processor";
import { ZyntraConsoleLogger } from "../services/logger.service";
import type { ZyntraProcedureContext } from "../types/procedure.interface";
import { ZyntraCookie } from "../services/cookie.service";
...
/**
 * Middleware executor processor for the Zyntra Framework.
 * Handles execution of global and action-specific middlewares.
 */
export class MiddlewareExecutorProcessor {
  private static _logger: ZyntraLogger;

  private static get logger(): ZyntraLogger {
    if (!this._logger) {
  ...
  static async executeGlobal(
    ...
    middlewares: ZyntraProcedure<unknown, unknown, unknown>[]
    ...
    for (let i = 0; i < middlewares.length; i++) {
      ...
      try {
        ...
        if (result instanceof ZyntraResponseProcessor) {
          this.logger.debug(
            "Middleware early return",
   ...
   * Maps ProcessedRequest to the format expected by ZyntraProcedureContext.
   *
   * @param context - The processed context
  ...
  private static buildProcedureContext(
    ...
    // Map ProcessedRequest to ZyntraProcedureContext.request structure
    ...
    if (!procedureRequest.cookies) {
      ...
      procedureRequest.cookies = new ZyntraCookie(procedureRequest.headers);
    }
    ...
    const procedureContext: ZyntraProcedureContext<any> = {
      request: procedureRequest,
      context: context.$context || {}, // Use $context, not the full context
      response: context.response || new ZyntraResponseProcessor(),
      next
    };
