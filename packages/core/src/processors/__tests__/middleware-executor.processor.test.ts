...
import type { ZyntraProcedure } from '../../types/procedure.interface';
...
// Mock ZyntraCookie
const createMockCookie = (): ZyntraCookie => {
  const mockHeaders = new Headers();
  const cookie = new ZyntraCookie(mockHeaders);
...
const createMockContext = (overrides: Partial<ProcessedContext> = {}): ProcessedContext => ({
  ...
  response: new ZyntraResponseProcessor(),
  $context: {
...
const createMockProcedure = (
  name: string,
  handler: ZyntraProcedure<unknown, unknown, unknown>['handler'],
  options: Partial<ZyntraProcedure<unknown, unknown, unknown>> = {}
): ZyntraProcedure<unknown, unknown, unknown> => ({
  name,
  handler,
  ...options,
});
...
describe('MiddlewareExecutorProcessor', () => {
  describe('buildProcedureContext', () => {
    test('should build correct procedure context from ProcessedContext', () => {
      ...
      expect(procedureContext).toEqual({
        request: {
          ...
          cookies: expect.any(ZyntraCookie),
        },
        context: context.$context,
        response: expect.any(ZyntraResponseProcessor),
      });
    });
    ...
    test('should create fallback cookies when missing', () => {
      ...
      expect(procedureContext.request.cookies).toBeInstanceOf(ZyntraCookie);
    });
  ...
  describe('executeGlobal', () => {
    ...
    test('should handle early return with ResponseProcessor', async () => {
      const context = createMockContext();
      const responseProcessor = new ZyntraResponseProcessor().unauthorized('Access denied');
