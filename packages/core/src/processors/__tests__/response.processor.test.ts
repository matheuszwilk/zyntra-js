...
describe('Response Processor', () => {
  let mockStore: ZyntraStoreAdapter;
  ...
  describe('Processor Creation and Configuration', () => {
    it('should create processor with static init method', () => {
      const processor = ZyntraResponseProcessor.init();

      expect(processor).toBeInstanceOf(ZyntraResponseProcessor);
    });

    it('should create processor with store adapter', () => {
      const processor = ZyntraResponseProcessor.init(mockStore);

      expect(processor).toBeInstanceOf(ZyntraResponseProcessor);
    });

    it('should create processor with context and store', () => {
      const processor = ZyntraResponseProcessor.init<TestContext>(mockStore, testContext);
  ...
  describe('Success Responses', () => {
    it('should create success response with data', async () => {
      ...
      const response = await ZyntraResponseProcessor.init()
        .success(testData)
        .toResponse();
  ...
  describe('Streaming Responses', () => {
    ...
    it('should handle existing channels', async () => {
      ...
      await ZyntraResponseProcessor.init()
        .stream({
  ...
  describe('Cache Revalidation', () => {
    it('should set revalidation with query keys array', async () => {
      const response = await ZyntraResponseProcessor.init<TestContext>(mockStore, testContext)
    ...
    it('should set revalidation with single query key', async () => {
      await ZyntraResponseProcessor.init<TestContext>(mockStore, testContext)
  ...
  describe('Data Serialization and Types', () => {
    it('should handle primitive data types', async () => {
      const responses = await Promise.all([
        ZyntraResponseProcessor.init().success('string')
            .toResponse(),
        ZyntraResponseProcessor.init().success(42)
            .toResponse(),
        ZyntraResponseProcessor.init().success(true)
            .toResponse(),
        ZyntraResponseProcessor.init().success(null)
            .toResponse(),
      ]);
  ...
  describe('Response Structure Consistency', () => {
    it('should always return consistent response structure for success', async () => {
      const responses = await Promise.all([
        ZyntraResponseProcessor.init().success({ test: 1 })
            .toResponse(),
        ZyntraResponseProcessor.init().created({ test: 2 })
            .toResponse(),
        ZyntraResponseProcessor.init().json({ test: 3 })
            .toResponse(),
      ]);
    ...
    it('should always return consistent response structure for errors', async () => {
      const responses = await Promise.all([
        ZyntraResponseProcessor.init().badRequest('Bad')
            .toResponse(),
        ZyntraResponseProcessor.init().unauthorized('Unauth')
            .toResponse(),
        ZyntraResponseProcessor.init().forbidden('Forbidden')
            .toResponse(),
        ZyntraResponseProcessor.init().notFound('NotFound')
            .toResponse(),
      ]);
    ...
    it('should maintain consistent headers', async () => {
      const responses = await Promise.all([
          ZyntraResponseProcessor.init().success({})
            .toResponse(),
        ZyntraResponseProcessor.init().badRequest()
            .toResponse(),
      ]);
