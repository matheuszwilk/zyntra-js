...
/**
 * Error handler processor for the Zyntra Framework.
...
export class ErrorHandlerProcessor {
  ...
  /**
   * Handles ZyntraError instances.
   *
   * @param error - The ZyntraError instance
   * @param context - The processed context
   * @param telemetrySpan - The telemetry span for tracking
   * @param startTime - Request start time
   * @returns Standardized error response
   */
  static async handleZyntraError(
    error: ZyntraError,
    context: ProcessedContext,
    telemetrySpan: TelemetrySpan | null,
    startTime: number
    ...
    this.logger.error("Zyntra framework error", {
      error: {
        code: normalizedError.code,
        message: normalizedError.message,
        details: normalizedError.details
    ...
    // Track zyntra error for CLI dashboard
    await this.trackError(context, startTime, statusCode, error);
  ...
  static async handleError(
    ...
    // ZyntraError instances
    if (error instanceof ZyntraError) {
      return this.handleZyntraError(error, context, telemetrySpan, startTime);
    }
  ...
  private static async trackError(
    ...
    try {
      ...
      const fallbackLogger = ZyntraConsoleLogger.create({
        level: ZyntraLogLevel.ERROR,
