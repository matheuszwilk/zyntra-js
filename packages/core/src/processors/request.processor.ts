...
import { ZyntraError } from "../error";
import { ZyntraResponseProcessor } from "./response.processor";
...
import {
  ...
  type HTTPMethod,
  type ZyntraAction,
  type ZyntraControllerConfig,
  type ZyntraLogger,
  type ZyntraProcedure,
  type ZyntraRouter,
} from "../types";
import type {
...
import { ZyntraRealtimeService } from "../services/realtime.service";
import { ZyntraConsoleLogger } from "../services/logger.service";
import { ZyntraPluginManager } from "../services/plugin.service";
import { resolveLogLevel, createLoggerContext } from "../utils/logger";

/**
 * Handles HTTP request processing for the Zyntra Framework.
...
> implements RequestProcessorInterface<TRouter, TConfig>
{
  ...
  public router: RouterContext<
    ZyntraAction<any, any, any, any, any, any, any, any, any, any>
  >;
  public pluginManager?: ZyntraPluginManager<any>;

  private logger: ZyntraLogger;
  ...
  private static get isInteractiveMode() {
    return process.env.ZYNTRA_INTERACTIVE_MODE === "true";
  }
  ...
  constructor(config: TConfig) {
    ...
    this.logger = ZyntraConsoleLogger.create({
      level: process.env.ZYNTRA_LOG_LEVEL as ZyntraLogLevel || ZyntraLogLevel.INFO,
      context: {
    ...
    this.router = createRouter<ZyntraAction<any, any, any, any, any, any, any, any, any, any>>();
  ...
  private initializePluginManager(): void {
    if (this.config.plugins && Object.keys(this.config.plugins).length > 0) {
      try {
        ...
        this.pluginManager = new ZyntraPluginManager({
          store,
          logger,
          config: {
  ...
  private registerRoutes(): void {
    const basePATH = this.config.basePATH || process.env.ZYNTRA_APP_BASE_PATH || "/api/v1";
    ...
    for (const controller of Object.values(
      this.config.controllers,
    ) as ZyntraControllerConfig<any>[]) {
      for (const endpoint of Object.values(controller.actions) as ZyntraAction<
        any,
        any,
        any,
        any,
    ...
    const sseAction: ZyntraAction<any, any, any, any, any, any, any, any, any, any> = {
      method: "GET",
      type: "query",
      path: "/sse/events",
  ...
  private registerPluginRoutes(basePATH: string): void {
    ...
    for (const pluginName of pluginNames) {
      ...
      for (const [controllerName, controllerActions] of Object.entries(plugin.$controllers)) {
        for (const [actionName, actionConfig] of Object.entries(controllerActions as any) as [string, ZyntraAction<any, any, any, any, any, any, any, any, any, any>][]) {
          try {
            ...
            const wrappedAction: ZyntraAction<any, any, any, any, any, any, any, any, any, any> = {
              ...actionConfig,
              ...
              // Ensure required ZyntraAction properties
  ...
  async process(request: Request): Promise<Response> {
    ...
    try {
      ...
      const basePATH =
        this.config.basePATH || process.env.ZYNTRA_APP_BASE_PATH || "/api/v1";
      const sseEndpoint = parseURL(basePATH, "/sse/events");
      ...
      if (handler.use && Array.isArray(handler.use)) {
        const actionResult = await MiddlewareExecutorProcessor.executeAction(
          context,
          handler.use as ZyntraProcedure<unknown, unknown, unknown>[],
        );
  ...
  private async executeAction(
    handler: ZyntraAction<any, any, any, any, any, any, any, any, any, any>,
    ...
    const responseProcessor = ZyntraResponseProcessor.init(
      context.$plugins?.store || context.$context?.store,
    );

    const realtimeService = new ZyntraRealtimeService(
      context.$plugins?.store || context.$context?.store
    );

    // Execute handler with proper ZyntraActionContext structure
  ...
  private async handleSuccessfulResponse(
    ...
    if (actionResponse instanceof ZyntraResponseProcessor) {
      this.logger.debug('Response processor returned', {
        type: 'ZyntraResponseProcessor'
      });
   ...
   *
   * @throws {ZyntraError} When controller or action not found
   */
  async call<
  ...
  >(
    ...
    const basePATH =
      this.config.basePATH || process.env.ZYNTRA_APP_PATH || "/api/v1";
    const baseURL =
      this.config.baseURL ||
      process.env.ZYNTRA_APP_URL ||
      "http://localhost:3000";
  ...
  static async process(context: any): Promise<any> {
    ...
    try {
      ...
      const deprecationLogger = ZyntraConsoleLogger.create({
      ...
      error = err as Error;
      statusCode = error instanceof ZyntraError ? 500 : 500;
      throw error;
    }
  }
}
