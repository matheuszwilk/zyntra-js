# AI Agent Maintenance Manual: `@zyntra-js/adapter-opentelemetry`

**Version:** 1.0.0
**For AI Agent Use Only.**

This document provides a comprehensive technical guide for Large Language Model (LLM) based AI agents responsible for maintaining, debugging, and extending the `@zyntra-js/adapter-opentelemetry` package. You are an expert TypeScript software engineer; this manual is your single source of truth for this package. Adherence to these instructions is critical.
...
### 1.2. Purpose
This package is an **Adapter**. Its sole responsibility is to implement the `ZyntraTelemetryProvider` interface (defined in `@zyntra-js/core`) using the **OpenTelemetry** standard. It provides a production-ready solution for observability, enabling distributed tracing, metrics collection, and structured event logging for ZyntraJS applications. This adapter makes it possible to monitor, debug, and analyze the performance of applications in complex, distributed environments.

---

## 2. Architecture & Key Concepts

To effectively maintain this package, you must understand its role as a sophisticated wrapper around the OpenTelemetry SDKs, tailored to the ZyntraJS lifecycle.

### 2.1. The Adapter Pattern and the Core Contract

The `ZyntraTelemetryProvider` interface, located in `packages/core/src/types/telemetry.interface.ts`, is the **canonical contract** for this package. The object returned by `createOpenTelemetryAdapter` must fully implement this interface. The key methods this adapter provides are:
...
### 2.3. Integration with the ZyntraJS Request Lifecycle

The true power of this adapter is unlocked when it's registered with the Zyntra Builder via the `.telemetry()` method. This allows it to hook into the core `RequestProcessor`.
...
2.  **Context Injection:** This is a critical step. The adapter injects the created OpenTelemetry `span` and its `traceContext` directly into the ZyntraJS `context`. This makes the current span available inside every `Procedure` and `Action` handler via `context.span`.
...
-   **`ZyntraSpan` (`span.ts`):** A wrapper around the OTel `Span` object. It provides a more fluent and convenient API for developers, such as `.setTag()`, `.setError()`, and `.child()`.
-   **`ZyntraTimer` (`timer.ts`):** A utility class returned by the `telemetry.timer()` method. It simplifies the process of measuring the duration of an operation and recording it as a histogram metric upon calling `.finish()`.
...
*   `src/span.ts`
    > **Purpose**: Defines the `ZyntraSpan` wrapper class. This class provides a developer-friendly API over the standard OpenTelemetry `Span` interface.
...
*   `src/timer.ts`
    > **Purpose**: Defines the `ZyntraTimer` utility class, which simplifies the process of timing an operation and recording a histogram metric.
...
This systematic, type-first, and test-driven approach is mandatory for maintaining the quality and reliability of the ZyntraJS adapters. Always consult the interfaces in `@zyntra-js/core` and the types within this package before implementing changes.
