...
OpenTelemetry adapter for ZyntraJS telemetry system. Provides production-ready observability with distributed tracing, metrics, and events.
...
import { createSimpleOpenTelemetryAdapter } from '@zyntra-js/adapter-opentelemetry';
...
export const router = zyntra.router({
...
```typescript
import { createProductionOpenTelemetryAdapter } from '@zyntra-js/adapter-opentelemetry';
...
```typescript
import { createOpenTelemetryAdapter } from '@zyntra-js/adapter-opentelemetry';
...
```typescript
export const complexOperation = zyntra.query({
...
```typescript
export const trackingHandler = zyntra.query({
  handler: async ({ context }) => {
    // Counters
...
```typescript
export const auditHandler = zyntra.mutation({
  handler: async ({ context, input }) => {
...
- [ZyntraJS Core Documentation](../core/README.md)
