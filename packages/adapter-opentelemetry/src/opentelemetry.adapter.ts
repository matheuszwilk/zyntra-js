import {
  type ZyntraTelemetryProvider,
  type ZyntraTelemetrySpan,
  type ZyntraTimer,
  type ZyntraSpanOptions,
  ZyntraConsoleLogger,
  ZyntraLogLevel,
} from '@zyntra-js/core';
...
/**
 * OpenTelemetry adapter implementation for Zyntra.js telemetry system
 */
export class OpenTelemetryAdapterImpl implements OpenTelemetryAdapter {
  ...
  private _activeSpan: ZyntraTelemetrySpan | null = null;
  private _logger = ZyntraConsoleLogger.create({
    level: process.env.NODE_ENV === 'production' ? ZyntraLogLevel.INFO : ZyntraLogLevel.DEBUG,
    context: {
  ...
  private _createNoOpSpan(name: string): ZyntraTelemetrySpan {
    return {
      name,
      id: 'noop',
      traceId: 'noop',
