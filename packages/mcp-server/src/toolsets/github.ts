...
export function registerGitHubTools({ server, octokit }: ToolsetContext) {
  ...
  server.registerTool("search_github_issues", {
    ...
    inputSchema: {
      ...
      repository: z.string().optional().describe("Repository in format 'owner/repo'. Defaults to 'matheuszwilk/zyntra-js'"),
    ...
    try {
      const [owner, repo] = (repository || 'matheuszwilk/zyntra-js').split('/');
