 ...
 * This toolset provides programmatic access to the Zyntra.js CLI, enabling agents to manage the development lifecycle,
...
export function registerCliTools({ server, execAsync }: ToolsetContext) {
  ...
  server.registerTool("start_dev_server", {
    ...
    description: `**What it does:** Starts the Zyntra.js development server, enabling live reloading, client generation, and interactive debugging.
...
**How it works:** It programmatically runs 'npm run dev', which often starts both the web framework (like Next.js) and the Zyntra.js client generator.
**Result:** A running development server, with output logs streamed to the response.`,
    ...
    try {
      ...
      const command = `npx @zyntra-js/cli@latest dev ${args.join(" ")}`.trim();
  ...
  server.registerTool("build_project", {
    title: "Build Project",
    description: `**What it does:** Compiles the project for production. This includes building the web framework and generating the final Zyntra.js client.
...
**Result:** A test report summarizing passed and failed tests.`,
    inputSchema: {
      filter: z.string().optional().describe("Filter tests by a specific pattern or package name (e.g., '@zyntra-js/core')."),
  ...
  server.registerTool("generate_feature", {
    ...
    description: `**What it does:** Scaffolds a complete, new feature module according to Zyntra.js conventions.
**When to use:** When starting a new, distinct area of functionality in the application (e.g., 'users', 'products', 'billing').
**How it works:** Runs 'zyntra generate feature <name>'. It creates a directory with subfolders for controllers, procedures, and types.
**Result:** A new feature directory and files, ready for business logic implementation.`,
    ...
    try {
      ...
      const command = `npx @zyntra-js/cli@latest generate feature ${args.join(" ")}`.trim();
...
**How it works:** Runs 'zyntra generate schema'. It introspects your main router file and outputs the client files.
**Result:** Updated client schema files in the specified output directory.`,
    ...
    try {
      ...
      const command = [
        `npx @zyntra-js/cli@latest generate schema`.trim(),
        `npx @zyntra-js/cli@latest generate docs`.trim()
      ];
