...
import { isServer, SchedulePatterns } from "@zyntra-js/core";
import type { BullMQAdapterOptions, BullMQInstances, BullMQQueue, BullMQJob } from "./types";
import { createJobsRouter, createJobsRegistry, createJobsProxy } from "@zyntra-js/core";
import type { StandardSchemaV1 } from "@zyntra-js/core";
import type { JobExecutionContext } from "@zyntra-js/core";
import { ZyntraError } from "@zyntra-js/core";

/**
 * Creates a Job Queue Adapter for BullMQ.
 * 
 * This adapter provides a unified interface for Zyntra to interact with BullMQ,
 ...
 * @returns A complete `ZyntraJobQueueAdapter` implementation
 * 
 * @example
 * ```typescript
 * import { createBullMQAdapter } from "@zyntra-js/core/adapters";
 * import { createRedisStoreAdapter } from "@zyntra-js/core/adapters";
 * import type { ZyntraAppContext } from "@/zyntra.context";
 * 
 * const redisStore = createRedisStoreAdapter(redisClient);
 * const jobQueue = createBullMQAdapter<ZyntraAppContext>({ store: redisStore });
 * 
 * const zyntra = Zyntra
 ...
 * ```
 */
export function createBullMQAdapter<TContext extends object>(options: BullMQAdapterOptions = {}): ZyntraJobQueueAdapter<TContext> {
  if (!isServer) {
    return {} as ZyntraJobQueueAdapter<TContext>;
  }  
    ...
    try {
      const response = await fetch(webhookUrl, {
        ...
        headers: {
          ...
          'User-Agent': 'Zyntra-Jobs-Webhook/1.0',
        },
        body: JSON.stringify({
          ...payload,
