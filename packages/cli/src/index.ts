...
import { ZyntraRouter } from "@zyntra-js/core";
...
program
  .name("zyntra")
  .description("CLI for Zyntra.js type-safe client generation")
  .version("1.0.0")
...
program
  .command("init")
  .description("Create a new Zyntra.js project with interactive setup")
...
program
  .command("dev")
  .description("Start development mode with framework and Zyntra (interactive dashboard and OpenAPI docs by default)")
  ...
  .option("--no-framework", "Disable framework dev server (Zyntra only)")
  ...
  .action(async (options) => {
    ...
    processes.push({
      name: "Zyntra",
      command: `npx @zyntra-js/cli@latest generate schema --watch --framework ${framework} --output ${options.output}${docsFlags}`,
      color: "blue",
      cwd: process.cwd()
    });
    if (useInteractive) {
...
generate
  .command("schema")
  .description("Generate client schema from your Zyntra router (for CI/CD or manual builds)")
  .option("--framework <type>", `Framework type (${getFrameworkList()}, generic)`)
  ...
  .action(async (options) => {
    ...
    const { ZyntraWatcher } = await import("./adapters/build/watcher");
    const watcher = new ZyntraWatcher({
      framework,
...
generate
  ...
  .action(async (options) => {
    ...
    try {
      ...
      let router: ZyntraRouter<any, any, any, any, any> | null = null;
      let foundRouterPath = '';
      ...
      if (!router) {
        timeline.fail('No Zyntra router found in your project. Please ensure you have a router file (e.g., src/zyntra.router.ts).');
        process.exit(1);
      }
