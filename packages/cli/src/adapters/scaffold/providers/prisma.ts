import { getSchema, Model, Property, Attribute, KeyValue, RelationArray } from '@mrleebo/prisma-ast';
...
export class PrismaProvider implements SchemaProvider {
  ...
  public async getModel(modelName: string): Promise<ModelSchema | null> {
    try {
      ...
      // First pass: Build a map of FK field names to their relation field names
      // e.g., for `author User @relation(fields: [authorId], references: [id])`, maps "authorId" → "author"
      const fkFieldMap = new Map<string, string>();
      for (const prop of model.properties) {
        if (prop.type === 'field' && hasAttribute(prop, 'relation')) {
          const fkFieldNames = this.extractRelationFkFields(prop);
          for (const fkName of fkFieldNames) {
            fkFieldMap.set(fkName, prop.name);
          }
        }
      }

      // Second pass: Build field definitions with FK and nullable metadata
      const fields: SchemaProviderField[] = model.properties
        ...
        .map((prop) => {
          ...
          const isArray = !!(prop as any).array;
          const hasRelationAttr = hasAttribute(prop, 'relation');
          const knownScalarTypes = ['String', 'Int', 'Float', 'Decimal', 'BigInt', 'Boolean', 'DateTime', 'Json', 'Bytes', 'Unsupported'];
          const isRelation = hasRelationAttr || (isArray && !knownScalarTypes.includes(fieldTypeStr));
          ...
          return {
            ...
            isForeignKey: fkFieldMap.has(prop.name),
            relationFieldName: fkFieldMap.get(prop.name),
          };
        });
  ...
  /**
   * Extracts the FK field names from a @relation attribute.
   * For example, `@relation(fields: [authorId], references: [id])` returns `['authorId']`.
   *
   * @param prop - The relation field property to inspect.
   * @returns An array of FK field names referenced by the relation.
   */
  private extractRelationFkFields(prop: Property): string[] {
    if (prop.type !== 'field' || !Array.isArray(prop.attributes)) return [];

    const relationAttr = prop.attributes.find(attr => attr.name === 'relation');
    if (!relationAttr || !Array.isArray(relationAttr.args)) return [];

    for (const arg of relationAttr.args) {
      const val = arg.value;
      if (val && typeof val === 'object' && 'type' in val && (val as any).type === 'keyValue') {
        const kv = val as KeyValue;
        if (kv.key === 'fields' && kv.value && typeof kv.value === 'object' && 'type' in (kv.value as any)) {
          const arrVal = kv.value as RelationArray;
          if (arrVal.type === 'array' && Array.isArray(arrVal.args)) {
            return arrVal.args.map(a => String(a));
          }
        }
      }
    }
