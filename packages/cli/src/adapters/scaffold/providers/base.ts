 ...
 * it into a standardized format that the Zyntra.js scaffolding engine
...
export interface SchemaProviderField {
  ...
  /**
   * True if this field is nullable (accepts null values).
   * In Prisma, this corresponds to fields marked with `?`.
   */
  isNullable: boolean;

  /**
   * True if this field is a foreign key that references another model.
   */
  isForeignKey: boolean;

  /**
   * The name of the relation field associated with this foreign key.
   * For example, if `authorId` is the FK, this would be `"author"`.
   */
