...
export class OpenAPIGenerator {
  ...
  public generate(router: IntrospectedRouter): OpenApiV3Document {
    ...
    const spec: OpenApiV3Document = {
      openapi: '3.0.0',
      info: this.docsConfig.info || { title: 'Zyntra API', version: '1.0.0' },
      servers,
  ...
  private buildPaths(router: IntrospectedRouter): Record<string, any> {
    ...
    for (const [controllerKey, controller] of Object.entries(router.controllers)) {
      for (const [actionKey, action] of Object.entries(controller.actions)) {
        ...
        let fullPath = `/${controller.path}/${action.path}`;

        // Normalize slashes (remove double slashes and trailing slashes)
        fullPath = fullPath.replace(/\/{2,}/g, '/');
        if (fullPath.length > 1 && fullPath.endsWith('/')) {
          fullPath = fullPath.slice(0, -1);
        }

        // Convert Zyntra/Express-style params (:id) to OpenAPI-style ({id})
        ...
        paths[openApiPath][action.method.toLowerCase()] = operation;
      }
    }
    return paths;
  }
