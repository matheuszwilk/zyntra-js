import { ZyntraLogLevel, createConsoleLogger } from '@zyntra-js/core';

/**
 * Get the log level from environment variables
 * Supports DEBUG=true for debug level and LOG_LEVEL for explicit level
 */
function getLogLevel(): ZyntraLogLevel {
  if (process.env.DEBUG === 'true') {
    return ZyntraLogLevel.DEBUG;
  }

  const envLevel = process.env.LOG_LEVEL?.toUpperCase();
  if (envLevel && envLevel in ZyntraLogLevel) {
    return ZyntraLogLevel[envLevel as keyof typeof ZyntraLogLevel];
  }

  return ZyntraLogLevel.INFO;
}
...
export function createChildLogger(
  ...
  options: { level?: ZyntraLogLevel; colorize?: boolean } = {}
) {
...
export function setupCliLogger(options: { debug?: boolean }) {
  if (options.debug) {
    logger.setLevel(ZyntraLogLevel.DEBUG);
