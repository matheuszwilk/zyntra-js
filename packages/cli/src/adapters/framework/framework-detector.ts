...
import { startZyntraWithFramework } from './concurrent-processes'
...
export interface DevServerOptions {
  ...
  cwd?: string
  /**
   * Use concurrently to run with Zyntra watcher
   * @default false
   */
  withZyntra?: boolean
  /**
   * Custom Zyntra watcher command
   */
  zyntraCommand?: string
}
...
export async function startDevServer(options: DevServerOptions = {}): Promise<ChildProcess | void> {
  ...
  // If withZyntra option is enabled, use concurrent processes
  if (options.withZyntra) {
    logger.info('Starting dev server with Zyntra watcher', {
      framework,
      packageManager,
      command,
      port,
      zyntraCommand: options.zyntraCommand
    });

    try {
      await startZyntraWithFramework({
        framework,
        frameworkCommand: command,
        cwd,
        port,
        debug: options.debug,
        zyntraWatcherCommand: options.zyntraCommand
      });
...
export async function startZyntraDev(options: {
  ...
  const logger = createChildLogger({ component: 'zyntra-dev' });
  ...
  logger.info('Starting Zyntra development mode', {
    framework,
    packageManager,
    frameworkCommand,
  ...
  try {
    ...
    logger.error('Failed to start Zyntra development', { error: formatError(error) });
    throw error;
  }
}