...
class InteractiveProcessManager {
  ...
  private zyntraTabIndex: number; // Index of the Zyntra tab
  ...
  constructor(configs: ProcessConfig[]) {
    // Reorganize configs: Framework first, then Zyntra
    this.processConfigs = this.reorganizeConfigs(configs);
    
    // Set tab indices based on new order
    this.zyntraTabIndex = 1; // Zyntra is always second
  ...
  /**
   * Reorganize configs to put framework first, then Zyntra
   */
  private reorganizeConfigs(configs: ProcessConfig[]): ProcessConfig[] {
    ...
    const zyntraConfig = configs.find(c => 
      c.name.toLowerCase().includes('zyntra')
    );
    
    const otherConfigs = configs.filter(c => c !== frameworkConfig && c !== zyntraConfig);
    
    const result = [];
    if (frameworkConfig) result.push(frameworkConfig);
    if (zyntraConfig) result.push(zyntraConfig);
  ...
  private getProjectInfo(): ProjectInfo {
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        // Try to find Zyntra version from dependencies
        const allDeps = {
        ...
        const zyntraVersion = allDeps['@zyntra-js/core'] || 
                              allDeps['@zyntra-js/cli'] || 
                              allDeps['zyntra-js'];
        
        return {
          name: packageJson.name || 'Unknown Project',
          zyntraVersion: zyntraVersion ? zyntraVersion.replace(/[\^~]/, '') : undefined
        };
      }
    } catch (error) {
  ...
  private async detectSystemCapabilities() {
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        // Detect Zyntra adapters
        if (allDeps['@zyntra-js/adapter-redis'] || allDeps['@zyntra-js/adapter-redis']) {
          this.systemCapabilities.detectedPackages.redisAdapter = true;
          this.systemCapabilities.hasRedisStore = true;
        }

        if (allDeps['@zyntra-js/adapter-bullmq'] || allDeps['@zyntra-js/adapter-bullmq']) {
  ...
  private async testRedisConnection() {
    try {
      ...
      await subscriber.subscribe('zyntra:api-requests', 'zyntra:telemetry');
      
      subscriber.on('message', (channel, message) => {
        if (channel === 'zyntra:api-requests') {
          try {
        ...
        } else if (channel === 'zyntra:telemetry') {
          try {
            const data = JSON.parse(message);
  ...
  private drawHeader() {
    ...
    const versionInfo = this.projectInfo.zyntraVersion ? `v${this.projectInfo.zyntraVersion}` : '';
    const projectName = this.projectInfo.name !== 'Unknown Project' ? ` - ${this.projectInfo.name}` : '';
    
    const title = `${ANSI_COLORS.bold}${ANSI_COLORS.blue}Zyntra.js ${versionInfo}${projectName}${ANSI_COLORS.reset}`;
  ...
  private drawTabs() {
    ...
    tabs.push(`${apiStyle}${apiColor}${apiIcon} ${this.apiTabIndex + 1}. Zyntra.js API${ANSI_COLORS.reset}`);
    ...
    tabs.push(`${jobsStyle}${jobsColor}${jobsIcon} ${this.jobsTabIndex + 1}. Zyntra.js Jobs${queueDisplay}${ANSI_COLORS.reset}`);
  ...
  private drawApiRequests() {
    ...
    if (visibleRequests.length === 0) {
      console.log(`${ANSI_COLORS.dim}No API requests yet. See docs at https://zyntrajs.com/docs/api${ANSI_COLORS.reset}\n`);
      return;
    }
  ...
  private drawJobsInterface() {
    if (!this.systemCapabilities.hasJobQueue) {
      ...
      console.log(`  2. Install adapter: ${ANSI_COLORS.cyan}npm install @zyntra-js/adapter-bullmq${ANSI_COLORS.reset}`);
      console.log(`  3. Configure in your Zyntra router:\n`);
      console.log(`${ANSI_COLORS.dim}     import { createBullMQAdapter } from '@zyntra-js/adapter-bullmq'${ANSI_COLORS.reset}`);
      console.log(`${ANSI_COLORS.dim}     const jobs = createBullMQAdapter({ store: redisStore })${ANSI_COLORS.reset}`);
      console.log(`${ANSI_COLORS.dim}     const zyntra = Zyntra.context().store(store).jobs(jobs).create()${ANSI_COLORS.reset}\n`);
      return;
    }
  ...
  private drawJobsContent() {
    ...
    if (visibleJobs.length === 0) {
      console.log(`${ANSI_COLORS.dim}No jobs found. See docs at zyntra.js.org/docs/jobs${ANSI_COLORS.reset}\n`);
      return;
    }
  ...
  async start() {
    ...
    process.env.ZYNTRA_INTERACTIVE_MODE = 'true';
...
/**
 * Create a process config for the Zyntra watcher
 */
export function createZyntraWatcherProcess(options: {
  cwd?: string;
  debug?: boolean;
}): ProcessConfig {
  return {
    name: 'Zyntra',
    command: `npx @zyntra-js/cli@latest generate schema --watch${options.debug ? ' --debug' : ''}`,
    color: PROCESS_COLORS.zyntra,
    cwd: options.cwd,
    env: {
...
/**
 * Start both Zyntra watcher and framework dev server concurrently
 */
export async function startZyntraWithFramework(options: {
  framework: SupportedFramework;
  frameworkCommand: string;
  cwd?: string;
  port?: number;
  debug?: boolean;
  zyntraWatcherCommand?: string;
}): Promise<void> {
  const logger = createChildLogger({ component: 'zyntra-with-framework' });
  
  logger.info('Starting Zyntra with framework', {
  ...
  // Override zyntra process command if provided
  if (options.zyntraWatcherCommand) {
    processes[0].command = options.zyntraWatcherCommand;
  }
