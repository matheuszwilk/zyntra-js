...
/**
 * File watcher for Zyntra controllers
 * Monitors controller files and regenerates client schema on changes
 */
export class ZyntraWatcher {
  private watcher: any = null
  private config: ZyntraBuildConfig
  ...
  constructor(config: ZyntraBuildConfig) {
    this.config = {
      extractTypes: true,
  ...
  private async regenerateSchema() {
    ...
    try {
      ...
      const possibleRouterPaths = [
        'src/zyntra.router.ts', 'src/zyntra.router.js',
        'src/router.ts', 'src/router.js',
        'zyntra.router.ts', 'zyntra.router.js',
        'router.ts', 'router.js'
      ]
      ...
      if (error instanceof RouterLoadError) {
        console.error(chalk.red.bold('\n✗ Error loading your Zyntra router:'));
