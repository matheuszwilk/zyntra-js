...
import { ZyntraRouter, ZyntraControllerConfig, ZyntraAction } from '@zyntra-js/core';
import zodToJsonSchema from 'zod-to-json-schema';
import { createRequire } from 'module';

// This file is responsible for dynamically loading and introspecting the user's Zyntra router.
 ...
 */
export function introspectRouter(router: ZyntraRouter<any, any, any, any, any>): { schema: IntrospectedRouter, stats: { controllers: number, actions: number } } {
  ...
  for (const [controllerName, controller] of Object.entries(router.controllers)) {
    ...
    if (typedController && typedController.actions) {
      for (const [actionName, action] of Object.entries(typedController.actions)) {
        ...
        const typedAction = action as ZyntraAction<any, any, any, any, any, any, any, any, any, any>;
 ...
 */
export async function loadRouter(routerPath: string): Promise<ZyntraRouter<any, any, any, any, any>> {
  ...
  try {
    ...
    throw new Error('Module was compiled and loaded, but no valid Zyntra router export was found.');
  } catch (error: any) {
