...
describe('loadRouter - error scenario from bug report', () => {
  it('should not throw "Cannot read properties of undefined (reading caller)" error', async () => {
    ...
    // when createZyntraClient tries to access router.caller
    
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zyntra-bug-test-'));
    ...
    try {
      ...
      const routerContent = `
        ...
        const exampleController = zyntra.controller({
          path: '/example',
          actions: {
            hello: zyntra.query({
              method: 'GET',
      ...
      const routerPath = path.join(tempDir, 'zyntra.router.ts');
      ...
      // This is what createZyntraClient does - it should not throw
      expect(() => {
