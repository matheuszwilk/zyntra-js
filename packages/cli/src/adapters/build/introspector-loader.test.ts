...
describe('loadRouter with external dependencies', () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zyntra-test-'));
    
    // Create a simple router file that imports from @zyntra-js/core
    const routerContent = `
      ...
      const testController = zyntra.controller({
        path: '/test',
        actions: {
          hello: zyntra.query({
            method: 'GET',
    ...
    routerPath = path.join(tempDir, 'zyntra.router.ts');
