...
export async function handleInitCommand(
  ...
  try {
    ...
    logger.info(`Starting Zyntra.js project initialization (existing project: ${isExistingProject})`, {
      targetDir,
      options,
    });
    ...
    if (isExistingProject) {
      ...
      console.log(chalk.dim("Let's add Zyntra.js to your existing project..."));
      console.log();
    }
...
async function checkDirectoryAndConfirm(
  ...
  try {
    ...
    if (isExistingProject) {
      const zyntraCoreFile = path.join(targetDir, 'src', 'zyntra.ts');
      if (await fs.stat(zyntraCoreFile).catch(() => null)) {
        console.log(chalk.yellow('⚠️  It looks like Zyntra.js might already be set up here.'));
...
export function showInitHelp(): void {
  console.log(chalk.bold('\n🔥 Zyntra.js Init Command\n'))

  console.log(chalk.bold('Usage:'))
  console.log('  zyntra init [project-name] [options]')
  console.log('  zyntra init . [options]            # Initialize in current directory')
  console.log()

  console.log(chalk.bold('Examples:'))
  console.log('  zyntra init my-api                 # Create new project')
  console.log('  zyntra init .                      # Initialize current directory')
  console.log('  zyntra init my-api --force         # Skip confirmation prompts')
  console.log()
