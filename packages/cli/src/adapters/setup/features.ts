 ...
 */
export const ZYNTRA_FEATURES: Record<string, FeatureDefinition> = {
  store: {
    key: 'store',
    name: 'Redis Store',
    description: 'Caching, sessions, and pub/sub messaging',
    dependencies: [
      { name: '@zyntra-js/adapter-redis', version: 'latest' },
  ...
  jobs: {
    ...
    dependencies: [
      { name: '@zyntra-js/adapter-redis', version: 'latest' },
      { name: '@zyntra-js/adapter-bullmq', version: 'latest' },
    ...
    envVars: [
      ...
      { key: 'ZYNTRA_JOBS_QUEUE_PREFIX', value: 'zyntra', description: 'Job queue prefix' }
    ]
  },
  ...
  mcp: {
    ...
    dependencies: [
      { name: '@zyntra-js/adapter-mcp-server', version: 'latest' },
    ...
    envVars: [
      { key: 'ZYNTRA_MCP_SERVER_BASE_PATH', value: '/api/mcp', description: 'MCP server base path' },
      { key: 'ZYNTRA_MCP_SERVER_TIMEOUT', value: '3600000', description: 'MCP session timeout in ms' },
      { key: 'REDIS_URL', value: 'redis://localhost:6379', description: 'Redis connection URL' },
      { key: 'REDIS_HOST', value: 'localhost', description: 'Redis host' },
      { key: 'REDIS_PORT', value: '6379', description: 'Redis port' }
    ]
  ...
  logging: {
    ...
    dependencies: [
      { name: '@zyntra-js/core', version: 'latest' }
    ],
    envVars: [
      { key: 'ZYNTRA_LOG_LEVEL', value: 'info', description: 'Logging level (debug, info, warn, error)' },
    ]
  },
  ...
  telemetry: {
    ...
    envVars: [
      { key: 'ZYNTRA_TELEMETRY_ENABLE_TRACING', value: 'true', description: 'Enable telemetry tracing' },
      { key: 'ZYNTRA_TELEMETRY_ENABLE_METRICS', value: 'true', description: 'Enable telemetry metrics' },
      { key: 'ZYNTRA_TELEMETRY_ENABLE_EVENTS', value: 'true', description: 'Enable telemetry metrics' },
      { key: 'ZYNTRA_TELEMETRY_ENABLE_CLI_INTEGRATION', value: 'true', description: 'Enable telemetry metrics' },
    ]
  }
}
...
export function getFeatureDependencies(enabledFeatures: string[]): {
  ...
  for (const featureKey of enabledFeatures) {
    const feature = ZYNTRA_FEATURES[featureKey]
    if (feature) {
