...
/**
 * API route handler for Zyntra.js using TanStack Start.
 *
 ...
 * to the Zyntra.js router for processing.
 *
...
/**
 * The core handler function that bridges TanStack Start and Zyntra.js.
 * @param request - The incoming `Request` object from the client.
 * @returns A standard `Response` object.
 */
const zyntraApiHandler = async ({ request }: { request: Request }) => {
  // 1. Pass the incoming request to the Zyntra router's handler.
  const zyntraResponse = await AppRouter.handler(request)

  // 2. The Zyntra handler returns a standardized response object.
  // We need to convert its body to a string for the Response constructor.
  const body = zyntraResponse.body ? JSON.stringify(zyntraResponse.body) : null

  // 3. Construct and return a standard Web API Response object.
  return new Response(body, {
    status: zyntraResponse.status,
    headers: zyntraResponse.headers,
  })
}
