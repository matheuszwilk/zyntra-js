...
The Store system follows ZyntraJS's adapter pattern, providing a unified interface (`ZyntraStoreAdapter`) that can be implemented by different storage backends. The Redis adapter (`@zyntra-js/adapter-redis`) is the production-ready implementation, but you can create custom adapters for in-memory storage, Memcached, or any other storage backend that fits your needs.
...
Get started with the Store adapter in minutes. This quick start guide shows you how to install dependencies, create the adapter, register it with Zyntra, and start using it in your actions. Follow these steps to add high-performance caching and Pub/Sub messaging to your application.
...
<Steps>
  ...
  <Step>
    ...
    Use `zyntra.store` to access the store service in your handlers:
      ...
      actions: {
        list: zyntra.query({
          ...
          handler: async ({ context, response }) => {
            // Check cache first
            const cached = await zyntra.store.get<User[]>('users:list');
            ...
            // Cache for 5 minutes
            await zyntra.store.set('users:list', users, { ttl: 300 });
...
<Accordions>
  <Accordion title="API Response Caching">
    ...
    ```typescript
    const cached = await zyntra.store.get<ApiResponse>('api:data');
    if (cached) return cached;
    
    const data = await fetchExpensiveData();
    await zyntra.store.set('api:data', data, { ttl: 3600 });
    return data;
    ```
  ...
  <Accordion title="Session Management">
    ...
    ```typescript
    await zyntra.store.set(`session:${userId}`, sessionData, { ttl: 86400 });
    ```
  ...
  <Accordion title="Rate Limiting">
    ...
    ```typescript
    const count = await zyntra.store.increment(`rate:${userId}`);
    if (count === 1) {
      await zyntra.store.expire(`rate:${userId}`, 60);
    }
    if (count > 100) {
