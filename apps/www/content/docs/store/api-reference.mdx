...
## ZyntraStoreAdapter Interface

The Store adapter implements the `ZyntraStoreAdapter` interface, which provides a unified API for various storage backends.

```typescript
interface ZyntraStoreAdapter<TClient extends unknown = unknown> {
...
**Example:**
```typescript
const user = await zyntra.store.get<User>('user:123');
if (user) {
...
```typescript
// Store without TTL
await zyntra.store.set('user:123', { name: 'John', email: 'john@example.com' });

// Store with TTL (expires in 1 hour)
await zyntra.store.set('user:123', userData, { ttl: 3600 });
```
...
await zyntra.store.set('complex:data', data);
```
...
**Example:**
```typescript
await zyntra.store.delete('user:123');
```
...
**Example:**
```typescript
const exists = await zyntra.store.has('user:123');
if (!exists) {
  // Key doesn't exist, fetch and cache
  const user = await fetchUser();
  await zyntra.store.set('user:123', user);
}
```
...
const count = await zyntra.store.increment('page:views');

// Second call: returns 2
const newCount = await zyntra.store.increment('page:views');
```
...
await zyntra.store.set('user:123', userData);
await zyntra.store.expire('user:123', 3600); // Expires in 1 hour

// Update TTL
await zyntra.store.expire('user:123', 7200); // Extend to 2 hours
```

**Common Patterns:**
```typescript
// Set expiration on first increment
const count = await zyntra.store.increment('counter');
if (count === 1) {
  await zyntra.store.expire('counter', 3600);
}

// Refresh expiration on access
const value = await zyntra.store.get('key');
if (value) {
  await zyntra.store.expire('key', 3600); // Refresh TTL
}
```
...
await zyntra.store.publish('user:created', userData);
await zyntra.store.publish('order:status:changed', orderData);
await zyntra.store.publish('tenant:123:events', eventData);
```
...
await zyntra.store.subscribe('events', callback1);
await zyntra.store.subscribe('events', callback2);
await zyntra.store.subscribe('events', callback3);
...
await zyntra.store.unsubscribe('notifications', handleNotification);
...
**Example:**
```typescript
await zyntra.store.set('key', value, { ttl: 3600 });
```
...
try {
  ...
  await zyntra.store.set('key', circular);
} catch (error) {
...
const getCached = async <T>(key: string, context: AppContext): Promise<T | null> => {
  try {
    return await zyntra.store.get<T>(key);
  } catch (error) {
...
// ❌ Bad: No type safety
const user = await zyntra.store.get('user:123');
```
...
await zyntra.store.set('session', data, { ttl: 3600 }); // 1 hour
await zyntra.store.set('config', data, { ttl: 86400 }); // 1 day

// ❌ Bad: Missing or inappropriate TTL
await zyntra.store.set('session', data); // Never expires
await zyntra.store.set('temp', data, { ttl: 31536000 }); // 1 year for temp data
```
...
await zyntra.store.set('user:123:profile', profileData);
await zyntra.store.set('order:456:status', statusData);

// ❌ Bad: Vague or unclear
await zyntra.store.set('data', someData);
await zyntra.store.set('key1', value);
```
