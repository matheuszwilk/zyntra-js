...
<Accordions>
  <Accordion title="Storing Values">
    ...
    handler: async ({ context }) => {
      ...
      await zyntra.store.set('user:123', { name: 'John Doe', email: 'john@example.com' });
      
      // Store with TTL (time-to-live) in seconds
      await zyntra.store.set('user:123', userData, { ttl: 3600 }); // Expires in 1 hour
  ...
  <Accordion title="Checking Key Existence">
    ...
    handler: async ({ context }) => {
      ...
      if (!exists) {
        ...
        await zyntra.store.set('user:123', user, { ttl: 3600 });
      }
    }
    ```
  ...
  <Accordion title="Deleting Keys">
    ...
    handler: async ({ context }) => {
      ...
      await zyntra.store.delete('user:123'); // Invalidate cache
    }
    ```
...
<Accordions>
  <Accordion title="Cache-Aside Pattern">
    ...
    const getUser = async (id: string, context: AppContext) => {
      // 1. Check cache
      const cached = await zyntra.store.get<User>(`user:${id}`);
      if (cached) {
        return cached;
      }
      ...
      // 3. Store in cache
      await zyntra.store.set(`user:${id}`, user, { ttl: 3600 });
  ...
  <Accordion title="Write-Behind Pattern">
    ...
    const createUser = async (data: UserInput, context: AppContext) => {
      ...
      // Cache immediately
      await zyntra.store.set(`user:${user.id}`, user, { ttl: 3600 });
...
await zyntra.store.set('key', value, { ttl: 1800 });

// Cache for 1 day
await zyntra.store.set('key', value, { ttl: 86400 });
```
...
await zyntra.store.expire('user:123', 3600); // Extend to 1 hour

// Refresh TTL after access
const user = await zyntra.store.get<User>('user:123');
if (user) {
  await zyntra.store.expire('user:123', 3600); // Refresh expiration
}
```
...
<Accordions>
    ...
    const warmCache = async (context: AppContext) => {
      ...
      await Promise.all(
        popularUsers.map(user =>
          zyntra.store.set(`user:${user.id}`, user, { ttl: 3600 })
        )
      );
    };
    ```
  ...
  <Accordion title="Cache Invalidation">
    ...
    const invalidateUserCache = async (userId: string, context: AppContext) => {
      ...
      await zyntra.store.delete(`user:${userId}`);
      
      // Delete related caches
      await zyntra.store.delete('users:list');
      await zyntra.store.delete(`user:${userId}:posts`);
      await zyntra.store.delete(`user:${userId}:profile`);
    };
    ```
  ...
  <Accordion title="Conditional Caching">
    ...
    const getExpensiveData = async (params: QueryParams, context: AppContext) => {
      ...
      // Check cache
      const cached = await zyntra.store.get<Data>(cacheKey);
      if (cached) {
        return cached;
      }
      ...
      if (data.status === 'success') {
        await zyntra.store.set(cacheKey, data, { ttl: 300 });
      }
...
await zyntra.store.set('complex:data', complexData, { ttl: 3600 });

// Retrieved and automatically parsed
const retrieved = await zyntra.store.get<typeof complexData>('complex:data');
```
...
<Accordions>
  <Accordion title="Batch Operations">
    ...
    const user1 = await zyntra.store.get('user:1');
    const user2 = await zyntra.store.get('user:2');
    const user3 = await zyntra.store.get('user:3');
    
    // ✅ Fast: Use Promise.all (parallel requests)
    const [user1, user2, user3] = await Promise.all([
      zyntra.store.get('user:1'),
      zyntra.store.get('user:2'),
      zyntra.store.get('user:3'),
    ]);
    ```
  ...
  <Accordion title="Appropriate TTL Values">
    ...
    await zyntra.store.set('stock:price', price, { ttl: 60 }); // 1 minute
    
    // Moderately volatile: Medium TTL
    await zyntra.store.set('user:profile', profile, { ttl: 3600 }); // 1 hour
    
    // Stable data: Long TTL
    await zyntra.store.set('config:app', config, { ttl: 86400 }); // 1 day
    ```
  ...
  <Accordion title="Memory Considerations">
    ...
    await zyntra.store.set('large:data', largeData, { ttl: 300 }); // 5 minutes
...
<Accordions>
      ...
      actions: {
        fetchData: zyntra.query({
          ...
          handler: async ({ request, context, response }) => {
            ...
            // Check cache
            const cached = await zyntra.store.get<ApiResponse>(cacheKey);
            if (cached) {
            ...
            if (data.status === 'success') {
              await zyntra.store.set(cacheKey, data, { ttl: 600 });
            }
  ...
  <Accordion title="Session Management">
    ...
    const createSession = async (userId: string, context: AppContext) => {
      ...
      await zyntra.store.set(`session:${sessionId}`, sessionData, { ttl: 86400 });
      return sessionId;
    };
    
    const getSession = async (sessionId: string, context: AppContext) => {
      return await zyntra.store.get<SessionData>(`session:${sessionId}`);
    };
    
    const deleteSession = async (sessionId: string, context: AppContext) => {
      await zyntra.store.delete(`session:${sessionId}`);
    };
    ```
