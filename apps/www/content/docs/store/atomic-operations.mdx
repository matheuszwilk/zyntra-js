...
handler: async ({ context }) => {
  // Increment a counter
  const newValue = await zyntra.store.increment('page:views');
...
<Accordions>
  <Accordion title="Page Views Counter">
    ...
    const trackPageView = async (pageId: string, context: AppContext) => {
      const views = await zyntra.store.increment(`page:${pageId}:views`);
      
      // Set expiration on first view
      if (views === 1) {
        await zyntra.store.expire(`page:${pageId}:views`, 86400); // 24 hours
      }
  ...
  <Accordion title="User Activity Counter">
    ...
    const trackUserActivity = async (userId: string, context: AppContext) => {
      ...
      const count = await zyntra.store.increment(key);
      
      // Reset daily
      if (count === 1) {
        await zyntra.store.expire(key, 86400); // Expires at midnight
      }
  ...
  <Accordion title="Distributed Counter">
    ...
    const incrementGlobalCounter = async (
      ...
      const value = await zyntra.store.increment(`counter:${counterName}`);
...
<Accordions>
  <Accordion title="Auto-Expiring Sessions">
    ...
    const createSession = async (userId: string, context: AppContext) => {
      ...
      // Store session
      await zyntra.store.set(`session:${sessionId}`, sessionData);
      
      // Auto-expire after 24 hours
      await zyntra.store.expire(`session:${sessionId}`, 86400);
      
      return sessionId;
    };

    const refreshSession = async (sessionId: string, context: AppContext) => {
      const session = await zyntra.store.get<SessionData>(`session:${sessionId}`);
  ...
  <Accordion title="Temporary Data Cleanup">
    ...
    const storeTemporaryData = async (
      ...
      await zyntra.store.set(key, data);
      await zyntra.store.expire(key, ttl);
  ...
  <Accordion title="Refresh Expiration on Access">
    ...
    const getWithRefresh = async <T>(
      ...
      const value = await zyntra.store.get<T>(key);
...
<Accordions>
  <Accordion title="Rate Limiting">
    ...
    const checkRateLimit = async (
      ...
      if (count === 1) {
        await zyntra.store.expire(key, windowSeconds);
      }
  ...
  <Accordion title="Sliding Window Rate Limiting">
    ...
    const slidingWindowRateLimit = async (
      ...
      // Set expiration
      if (count === 1) {
        await zyntra.store.expire(key, windowSeconds * 2); // Keep 2 windows
      }
      ...
      const [currentCount, previousCount] = await Promise.all([
        zyntra.store.get<number>(`rate:sliding:${identifier}:${currentWindow}`) || 0,
        zyntra.store.get<number>(`rate:sliding:${identifier}:${previousWindow}`) || 0,
      ]);
  ...
  <Accordion title="Distributed Locks">
    ...
    const acquireLock = async (
      ...
      const exists = await zyntra.store.has(key);
      if (exists) {
        return { acquired: false };
      }
      
      // Set lock with expiration
      await zyntra.store.set(key, lockId);
      await zyntra.store.expire(key, ttl);
    ...
    const releaseLock = async (
      ...
      const key = `lock:${lockName}`;
      const storedLockId = await zyntra.store.get<string>(key);
      
      if (storedLockId === lockId) {
        await zyntra.store.delete(key);
        return true;
      }
  ...
  <Accordion title="Request Counting">
    ...
    const trackRequest = async (
      ...
      if (count === 1) {
        ...
        await zyntra.store.expire(key, secondsUntilMidnight);
      }
...
<Accordions>
  <Accordion title="Initialize Counters Properly">
    ...
    if (!(await zyntra.store.has('counter'))) {
      await zyntra.store.set('counter', 0);
    }
    const count = await zyntra.store.increment('counter');
    ```
  ...
  <Accordion title="Use Appropriate TTL Values">
    ...
    await zyntra.store.expire('session:123', 86400); // 24 hours for sessions
    await zyntra.store.expire('rate:limit:ip', 60);  // 1 minute for rate limits
    await zyntra.store.expire('cache:data', 3600);   // 1 hour for cache

    // ❌ Bad: Inappropriate TTL
    await zyntra.store.expire('session:123', 31536000); // 1 year for session (too long)
    await zyntra.store.expire('rate:limit:ip', 86400);  // 1 day for rate limit (too long)
    ```
  ...
  <Accordion title="Handle Race Conditions">
    ...
    const current = await zyntra.store.get<number>('counter') || 0;
    await zyntra.store.set('counter', current + 1);
...
const safeIncrement = async (
  ...
  try {
    return await zyntra.store.increment(key);
  } catch (error) {
...
<Accordions>
  <Accordion title="Batch Operations">
    ...
    await zyntra.store.increment('counter');
    await zyntra.store.expire('counter', 3600);
    ```
  ...
  <Accordion title="Monitoring Atomic Operations">
    ...
    const incrementWithMetrics = async (
      ...
      const value = await zyntra.store.increment(key);
...
<Accordions>
  <Accordion title="API Usage Tracking">
    ...
    const trackApiUsage = async (
      ...
      if (count === 1) {
        ...
        await zyntra.store.expire(key, secondsUntilNextMonth);
      }
  ...
  <Accordion title="Feature Flags with Usage Limits">
    ...
    const checkFeatureUsage = async (
      ...
      const usage = await zyntra.store.increment(key);
      
      // Set daily expiration
      if (usage === 1) {
        await zyntra.store.expire(key, 86400);
      }
