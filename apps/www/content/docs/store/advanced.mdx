...
<Accordions>
  ...
  <Accordion title="Fallback Strategies">
    ...
    const getWithFallback = async <T>(
      ...
      try {
        // Try cache first
        const cached = await zyntra.store.get<T>(key);
        if (cached) {
          return cached;
        }
        ...
        // Cache result
        await zyntra.store.set(key, data, { ttl: 3600 });
...
<Accordions>
  ...
  <Accordion title="Compression">
    ...
    const setCompressed = async (
      ...
      await zyntra.store.set(key, compressed.toString('base64'), options);
    };
    ...
    const getCompressed = async <T>(
      ...
      const compressed = await zyntra.store.get<string>(key);
  ...
  <Accordion title="Health Checks">
    ...
    const checkStoreHealth = async (
      ...
      try {
        ...
        await zyntra.store.has('health:check');
...
<Accordions>
  <Accordion title="Tenant Isolation">
    ...
    // Usage
    const tenantStore = new TenantStore(zyntra.store, request.tenantId);
...
<Accordions>
  <Accordion title="Use Appropriate TTLs">
    ...
    await zyntra.store.set('config:app', config, { ttl: 86400 }); // Stable data
    await zyntra.store.set('user:session', session, { ttl: 3600 }); // Session data
    await zyntra.store.set('api:rate', count, { ttl: 60 }); // Volatile data

    // ❌ Bad: Inappropriate TTLs
    await zyntra.store.set('user:session', session, { ttl: 31536000 }); // Too long
    await zyntra.store.set('config:app', config, { ttl: 60 }); // Too short
    ```
