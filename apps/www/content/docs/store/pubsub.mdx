...
<Accordions>
  <Accordion title="Publishing Messages">
    ...
    handler: async ({ context }) => {
      ...
      await zyntra.store.publish('notifications', {
        userId: '123',
  ...
  <Accordion title="Subscribing to Channels">
    ...
    const setupSubscriptions = async (context: AppContext) => {
      await zyntra.store.subscribe('notifications', async (message) => {
  ...
  <Accordion title="Unsubscribing">
    ...
    await zyntra.store.subscribe('notifications', callback);
    
    // Later, unsubscribe
    await zyntra.store.unsubscribe('notifications', callback);
    
    // Or unsubscribe all callbacks for a channel
    await zyntra.store.unsubscribe('notifications');
    ```
...
<Accordions>
  <Accordion title="Event Broadcasting">
      ...
      actions: {
        update: zyntra.mutation({
          ...
          handler: async ({ request, context, response }) => {
            ...
            await zyntra.store.publish('user:updated', {
              userId: user.id,
    ...
    await zyntra.store.subscribe('user:updated', async (event) => {
      // Update search index
  ...
  <Accordion title="Real-Time Notifications">
    ...
    const sendNotification = async (
      ...
      await zyntra.store.publish(`notifications:${userId}`, {
        userId,
        message,
        read: false,
    ...
    const setupUserNotifications = async (userId: string, context: AppContext) => {
      await zyntra.store.subscribe(`notifications:${userId}`, async (notification) => {
  ...
  <Accordion title="Cross-Service Communication">
    ...
    await zyntra.store.publish('order:created', {
      orderId: '123',
      userId: '456',
      items: [...],
      total: 99.99,
    ...
    await zyntra.store.subscribe('order:created', async (order) => {
      // Calculate shipping
      const shipping = await calculateShipping(order);
      
      // Publish to next service
      await zyntra.store.publish('order:shipping:calculated', {
    ...
    await zyntra.store.subscribe('order:shipping:calculated', async (data) => {
...
<Accordions>
  <Accordion title="Fan-Out Pattern">
    ...
    const publishUserEvent = async (event: UserEvent, context: AppContext) => {
      ...
      await zyntra.store.publish('events:all', event);
      
      // Publish to user-specific channel
      await zyntra.store.publish(`events:user:${event.userId}`, event);
      
      // Publish to feature-specific channel
      if (event.type === 'profile_updated') {
        await zyntra.store.publish('events:profile', event);
      }
    };
    ```
  ...
  <Accordion title="Filtering and Routing">
    ...
    await zyntra.store.subscribe('events:all', async (event) => {
  ...
  <Accordion title="Request-Response Pattern">
      ...
      return new Promise((resolve) => {
        ...
        const responseCallback = async (response: any) => {
          if (response.correlationId === correlationId) {
            await zyntra.store.unsubscribe(`response:${correlationId}`, responseCallback);
            resolve(response.data);
          }
        };
        
        zyntra.store.subscribe(`response:${correlationId}`, responseCallback);
        
        // Publish request
        zyntra.store.publish('requests:data', {
          correlationId,
          query,
        });
      });
    ...
    // Responder
    await zyntra.store.subscribe('requests:data', async (request) => {
      const data = await fetchData(request.query);
      
      await zyntra.store.publish(`response:${request.correlationId}`, {
...
<Accordions>
  <Accordion title="Basic Error Handling">
    ...
    ```typescript
    await zyntra.store.subscribe('orders:created', async (order) => {
      try {
        ...
        await zyntra.store.publish('errors:orders', {
          orderId: order.id,
          error: error.message,
          timestamp: new Date().toISOString(),
        });
  ...
  <Accordion title="Retry Logic">
    ...
    await zyntra.store.subscribe('critical:events', async (message) => {
...
<Accordions>
  <Accordion title="Application Startup">
    ...
    export const initializeSubscriptions = async (context: AppContext) => {
      // User events
      await zyntra.store.subscribe('user:created', handleUserCreated);
      await zyntra.store.subscribe('user:updated', handleUserUpdated);
      await zyntra.store.subscribe('user:deleted', handleUserDeleted);
      
      // Order events
      await zyntra.store.subscribe('order:created', handleOrderCreated);
      await zyntra.store.subscribe('order:status:changed', handleOrderStatusChanged);
      
      // Notifications
      await zyntra.store.subscribe('notifications', handleNotification);
  ...
  <Accordion title="Graceful Shutdown">
    ...
    const cleanupSubscriptions = async (context: AppContext) => {
      ...
      await zyntra.store.unsubscribe('user:created');
      await zyntra.store.unsubscribe('user:updated');
      await zyntra.store.unsubscribe('order:created');
      // ... other subscriptions
...
<Accordions>
  <Accordion title="Activity Feed">
    ...
    const recordActivity = async (
      ...
      await zyntra.store.publish(`feed:${userId}`, {
        userId,
        activity,
        timestamp: new Date().toISOString(),
      });
      ...
      await zyntra.store.publish('feed:global', {
        userId,
        activity,
        timestamp: new Date().toISOString(),
      });
    ...
    await zyntra.store.subscribe('feed:global', async (activity) => {
      // Update cached feed
      const feed = await zyntra.store.get<Activity[]>('feed:global:cached') || [];
      feed.unshift(activity);
      
      // Keep only last 100 activities
      const limitedFeed = feed.slice(0, 100);
      await zyntra.store.set('feed:global:cached', limitedFeed, { ttl: 3600 });
    });
    ```
  ...
  <Accordion title="Cache Invalidation">
    ...
    const updateProduct = async (
      ...
      await zyntra.store.publish('cache:invalidate', {
        keys: [
          `product:${productId}`,
          'products:list',
    ...
    await zyntra.store.subscribe('cache:invalidate', async (event) => {
      await Promise.all(
        event.keys.map(key => zyntra.store.delete(key))
      );
    });
    ```
  ...
  <Accordion title="Multi-Tenant Events">
    ...
    const publishTenantEvent = async (
      ...
      await zyntra.store.publish(`tenant:${tenantId}:events`, event);
    };
    ...
    const setupTenantSubscriptions = async (
      ...
      await zyntra.store.subscribe(`tenant:${tenantId}:events`, async (event) => {
...
<Accordions>
  <Accordion title="Use Meaningful Channel Names">
    ...
    await zyntra.store.publish('order:created', orderData);
    await zyntra.store.publish('user:profile:updated', profileData);
    
    // ❌ Bad: Vague or unclear
    await zyntra.store.publish('event', data);
    await zyntra.store.publish('update', data);
    ```
  ...
  <Accordion title="Structure Your Messages">
    ...
    await zyntra.store.publish('order:created', 'some data');
    ```
  ...
  <Accordion title="Handle Errors Gracefully">
    ...
    ```typescript
    // ✅ Good: Error handling
    await zyntra.store.subscribe('events', async (message) => {
      try {
        await processMessage(message);
      } catch (error) {
...
<Accordions>
  <Accordion title="Messages Not Received">
    ...
    **1. Verify subscription is active:**
    ...
    await zyntra.store.subscribe('channel', callback);
    await zyntra.store.publish('channel', message);
    ```
    
    **2. Check channel names match:**
    ```typescript
    // Publisher and subscriber must use exact same channel name
    await zyntra.store.publish('notifications', message);
    await zyntra.store.subscribe('notifications', callback); // ✅ Match
    ```
    
    **3. Verify Redis connection:**
    ```typescript
    const redis = (zyntra.store.client as Redis);
