...
export const users = zyntra.controller({
  name: 'Users',
...
<Tabs items={['npm', 'pnpm', 'yarn', 'bun']} groupId="package-manager">
  ...
  <Tab value="pnpm">
    ```bash
    pnpm @zyntra-js/cli@latest init
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn @zyntra-js/cli@latest init
    ```
  </Tab>
  <Tab value="bun">
    ```bash
...
<Callout type="info" title="Comparing Frameworks?">
  See how ZyntraJS compares to tRPC, Next.js Server Actions, and other frameworks.
...
ZyntraJS provides a complete AI development ecosystem that transforms how you build applications.

### Lia - Your AI Development Partner

**Lia** is a specialized Code Agent trained exclusively for the ZyntraJS ecosystem. Unlike generic AI assistants that struggle with framework-specific patterns, Lia is purpose-built to understand every aspect of ZyntraJS—from architecture decisions to testing strategies.
...
- **ZyntraJS Architecture** - Feature-based design, controller patterns, and type-safe APIs
...
- Scaffold complete features following ZyntraJS conventions
...
The **ZyntraJS MCP Server** is what powers Lia's advanced capabilities. It's a collection of specialized tools that transform your API into an AI-accessible workspace, enabling Lia to perform complex development tasks autonomously.

<Accordions>
  <Accordion title="Development Tools">
    These tools enable Lia to scaffold and build features following ZyntraJS conventions. She can generate complete, production-ready code structures without requiring manual boilerplate.
...
Installing Lia is simple. Use the ZyntraJS CLI to set up Lia in your preferred Code Agent:

<Tabs items={['Cursor', 'Claude Desktop', 'GitHub Copilot']}>
  <Tab value="Cursor">
    ```bash
    npx @zyntra-js/cli@latest lia --cursor
    ```
  ...
  <Tab value="Claude Desktop">
    ```bash
    npx @zyntra-js/cli@latest lia --claude
    ```
  ...
  <Tab value="GitHub Copilot">
    ```bash
    npx @zyntra-js/cli@latest lia --copilot
    ```
    
    Configures Lia for GitHub Copilot with training files and MCP integration. Enhances Copilot's understanding of ZyntraJS patterns.
  </Tab>
</Tabs>
...
<Accordions>
  <Accordion title="Feature Development">
    Lia can scaffold complete features with all necessary components—controllers, actions, types, and tests—following ZyntraJS best practices.
  ...
  <Accordion title="Project Management">
    ...
    **Examples:**
    ...
    - "Show me the implementation of `zyntra.query` in the core package"
    - "What are the current tasks and their status?"
  </Accordion>
</Accordions>

Lia understands the full context of your project and delivers production-ready code that follows ZyntraJS best practices.

### LLMs.txt

ZyntraJS website exposes multiple llms-readable documentation formats that help AI understand your API structure and how to interact with it.

#### Main Entry Point

**`/llms.txt`** - A comprehensive index that provides:
- Overview of the ZyntraJS framework
...
Visit [https://zyntrajs.com/llms.txt](https://zyntrajs.com/llms.txt) to see the complete index.
...
- `/blog/introducing-zyntra-js.mdx` - Blog post as markdown
...
All these formats are designed to make ZyntraJS documentation easily accessible to AI models, enabling better code generation, documentation understanding, and API interaction.