---
title: Procedures
description: Create reusable middleware with ZyntraJS procedures to handle authentication, validation, logging, and cross-cutting concerns with full type safety.
---
...
```typescript
const requestLoggerProcedure = zyntra.procedure({
...
<TypeTable type={{
  ...
  context: {
    type: 'AppContext',
    description: 'Application context from Zyntra.context()',
    required: true
  },
  response: {
...
const permissionsProcedure = zyntra.procedure({
  handler: async ({ context }) => {
...
```typescript
const requireRoleProcedure = zyntra.procedure({
  handler: async ({ context, response }) => {
...
const validateApiKeyProcedure = zyntra.procedure({
...
```typescript
const rateLimitProcedure = zyntra.procedure({
...
```typescript
const tracingProcedure = zyntra.procedure({
  handler: async ({ request, context }) => {
...
```typescript
const sanitizeInputProcedure = zyntra.procedure({
  handler: async ({ request }) => {
...
```typescript
const procedure1 = zyntra.procedure({
  handler: async () => ({ step1: 'done' })
});

const procedure2 = zyntra.procedure({
  handler: async ({ context }) => {
...
const procedure3 = zyntra.procedure({
  handler: async ({ context }) => {
...
```typescript
const optionalAuthProcedure = zyntra.procedure({
  handler: async ({ request }) => {
    const token = request.headers.get('authorization');
...
const publicAction = zyntra.query({
  use: [optionalAuth],
  handler: async ({ context }) => {
...
const authAndPermissionsProcedure = zyntra.procedure({
  handler: async ({ request, context }) => {
    const user = await authenticateUser(request);
