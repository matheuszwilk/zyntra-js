---
title: Controllers
description: Learn how to organize actions into logical controllers with shared paths, group related endpoints, and build modular APIs with ZyntraJS.
---
...
const userController = zyntra.controller({
  ...
  path: '/users',
  actions: {
    list: zyntra.query({ name: 'List Users', path: '/', handler: ... }),
    getById: zyntra.query({ name: 'Get User', path: '/:id' as const, handler: ... }),
    create: zyntra.mutation({ name: 'Create User', path: '/', method: 'POST', handler: ... }),
    update: zyntra.mutation({ name: 'Update User', path: '/:id' as const, method: 'PUT', handler: ... }),
    delete: zyntra.mutation({ name: 'Delete User', path: '/:id' as const, method: 'DELETE', handler: ... })
  }
});
```
...
```typescript
const postController = zyntra.controller({
  name: 'Posts',
  description: 'Manage blog posts and articles',
  path: '/posts',  // ← Base path
  actions: {
    list: zyntra.query({ name: 'List Posts', path: '/', ... }),        // → GET /posts/
    getById: zyntra.query({ name: 'Get Post', path: '/:id' as const, ... }),  // → GET /posts/:id
    create: zyntra.mutation({ name: 'Create Post', path: '/', ... })    // → POST /posts/
  }
});
```
...
<Callout type="success" title="OpenAPI Documentation">
  The `name` and `description` properties are crucial for generating comprehensive OpenAPI specifications. They enable the CLI to create detailed documentation that powers **Zyntra Studio** (our interactive API playground), making your API much more discoverable and user-friendly.
</Callout>
...
<TypeTable type={{
  ...
  actions: {
    type: 'Record<string, ZyntraAction>',
...
```typescript
const postCommentController = zyntra.controller({
  name: 'Post Comments',
...
const productController = zyntra.controller({
  name: 'Products',
...
const protectedController = zyntra.controller({
  name: 'Protected',
  description: 'Protected endpoints requiring authentication',
  path: '/protected',
  actions: {
    getData: zyntra.query({
...
export const userControllerV1 = zyntra.controller({
  name: 'Users API v1',
...
export const userControllerV2 = zyntra.controller({
  name: 'Users API v2',
...
```typescript
// ✅ Good - Domain-based
const userController = zyntra.controller({ path: '/users', ... });
const postController = zyntra.controller({ path: '/posts', ... });
const commentController = zyntra.controller({ path: '/comments', ... });

// ❌ Bad - Layer-based
const readController = zyntra.controller({ path: '/read', ... });
const writeController = zyntra.controller({ path: '/write', ... });
```
...
const apiController = zyntra.controller({
  path: '/api',
  actions: {
    getUsers: ...,
    getPosts: ...,
...
```typescript
const resourceController = zyntra.controller({
  name: 'Resources',
...
```typescript
const adminOnlyProcedure = zyntra.procedure({
...
const adminController = zyntra.controller({
  path: '/admin',
  actions: {
    getUsers: zyntra.query({
      use: [adminOnlyProcedure],
      handler: ...
    }),
    deleteUser: zyntra.mutation({
      use: [adminOnlyProcedure],
      handler: ...
    })
  }
...
This updates:
- `src/zyntra.schema.ts` - Type definitions for your client
