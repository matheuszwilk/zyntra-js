...
The **Router** is the central entrypoint for all HTTP requests in your ZyntraJS application. It maps incoming requests to your controllers and actions, providing both HTTP handling and type-safe server-side invocation.
...
The simplest way to create a router is with the `zyntra.router()` method:
...
console.log(AppRouter.config.baseURL);   // From Zyntra config
console.log(AppRouter.config.basePATH);  // From Zyntra config
```
...
// Result: The context type defined in Zyntra.context<T>()
```
...
// Result: The plugins type defined in Zyntra.plugins<T>()
```
...
```typescript
// v1 router
export const AppRouterV1 = zyntra.router({
  controllers: {
...
// v2 router
export const AppRouterV2 = zyntra.router({
  controllers: {
...
```typescript
// Public API router
export const PublicRouter = zyntra.router({
  controllers: {
    auth: authController,
...
// Admin API router
export const AdminRouter = zyntra.router({
  controllers: {
