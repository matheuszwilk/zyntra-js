---
title: Quick Start
description: Build your first type-safe API with ZyntraJS in 10 minutes. Learn the core concepts by creating a complete user management API.
---
...
By the end, you'll have a working API and understand the core ZyntraJS patterns.

## Prerequisites

Before starting, ensure you have:

- ZyntraJS installed ([Installation Guide](/docs/installation))
- Basic TypeScript knowledge
- A code editor with TypeScript support

## Quick Start Tutorial

<Card title="Complete ZyntraJS Setup Guide">
  <Steps>
    <Step>
      ### Create Your First Router

      Let's start by creating the core Zyntra instance. This is where you configure your application's context and settings.

      Create `src/zyntra.ts`:
      ...
      // Create and configure your Zyntra instance
      ...
      <Callout type="info" title="What's Happening Here?">
        - **`Zyntra.context(createZyntraAppContext())`** - Passes the context factory function to define your application's global context
        - **`.config()`** - Sets the base URL and path for your API
        - **`.create()`** - Creates the configured Zyntra instance with all methods available
      </Callout>
    </Step>
    ...
    <Step>
      ...
      <Callout type="success" title="OpenAPI Documentation">
        The `name` and `description` properties are crucial for generating comprehensive OpenAPI specifications for your API. They enable the CLI to create detailed documentation that powers **Zyntra Studio** (our interactive API playground), making your API much more discoverable and user-friendly.
      </Callout>
    </Step>
...
Actions are the actual API endpoints. ZyntraJS has two types:
...
search: zyntra.query({
  ...
  handler: async ({ request, context, response }) => {
    ...
    <Step>
      ...
      export const client = createZyntraClient<typeof AppRouter>({
        baseURL: 'http://localhost:3000',
        basePATH: '/api/v1',
        router: AppRouter
      });
      ...
      <Callout type="info" title="Type Inference Magic">
        The `createZyntraClient` function infers all your API types automatically. You don't need to manually define any types!
      </Callout>
    </Step>
...
Congratulations! You've built your first ZyntraJS API. You now understand:

✅ **The Builder Pattern** - How to configure your Zyntra instance  
...
This regenerates:
- `src/zyntra.schema.ts` - Updated type definitions for your client
...
ZyntraJS automatically catches errors and formats them:
...
<Callout type="success">
  You're now ready to build production-grade APIs with ZyntraJS! Check out the [Core Concepts](/docs/builder) to dive deeper.
</Callout>
