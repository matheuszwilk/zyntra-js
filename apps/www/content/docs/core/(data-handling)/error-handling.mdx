...
description: Master error handling in ZyntraJS with ZyntraError, ZyntraResponseError, validation errors, custom error codes, and global error handlers.
---

## Overview

**Error Handling** in ZyntraJS is designed to be type-safe, structured, and developer-friendly. The framework provides built-in error classes, automatic validation error handling, and comprehensive error tracking through telemetry.

```typescript
import { ZyntraError, ZyntraResponseError } from '@zyntra-js/core';
...
const getUser = zyntra.query({
  ...
  handler: async ({ request, response, context }) => {
    ...
    // ✅ Or throw ZyntraError for framework-level errors
    if (!user.isActive) {
...
<Callout type="success" title="Automatic Error Handling">
  ZyntraJS automatically catches and formats all errors, including validation errors, framework errors, and uncaught exceptions.
</Callout>
...
```typescript
type ZyntraResponse<TData, TError> = 
...
ZyntraJS provides semantic error response methods:

```typescript
const action = zyntra.query({
...
The `ZyntraResponseError` class represents client-facing errors:

### Creating Response Errors

```typescript
import { ZyntraResponseError } from '@zyntra-js/core';

const action = zyntra.mutation({
  handler: async ({ response }) => {
    const error = new ZyntraResponseError({
...
ZyntraJS defines standard error codes:
...
error.toString(); 
// 'ZyntraResponseError [ERR_NOT_FOUND]: User not found'
```

---

## ZyntraError (Framework Errors)

The `ZyntraError` class is for **framework-level** errors (not client-facing):
...
### ZyntraError Properties

```typescript
class ZyntraError extends Error {
...
```typescript
const getRecommendations = zyntra.query({
  handler: async ({ response, context }) => {
    try {
...
ZyntraJS has a built-in `ErrorHandlerProcessor` that handles all errors:
...
2. **ZyntraError** (Framework errors)
...
try {
  ...
  if (error instanceof ZyntraError) {
    // Framework error → 500
    return ErrorHandlerProcessor.handleZyntraError(error);
  }
...
const action = zyntra.query({
  handler: async ({ response }) => {
    const error = new ZyntraResponseError<CustomErrorCode>({
      code: 'USER_SUSPENDED',
...
export const createUserError = (
  ...
  return new ZyntraResponseError({
    code: `USER_${code}`,
  ...
  handler: async ({ response, context }) => {
    try {
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        return response.error(new ZyntraResponseError({
...
```typescript
const getResource = zyntra.query({
  path: '/:id' as const,
  handler: async ({ request, response, context }) => {
...
```typescript
const deletePost = zyntra.mutation({
...
```typescript
const getExternalData = zyntra.query({
  handler: async ({ response }) => {
    try {
