...
description: Master all response types in ZyntraJS including success, error, streaming, cookies, headers, and cache revalidation for building robust APIs.
---

## Overview

**Response Types** in ZyntraJS provide a comprehensive, type-safe API for constructing HTTP responses. The `response` object available in action handlers gives you full control over status codes, headers, cookies, body content, streaming, and client-side cache revalidation.
...
// result is typed as: ZyntraResponse<{ users: User[] }>
```
...
```typescript
const deleteUser = zyntra.mutation({
...
```typescript
const protectedAction = zyntra.query({
...
```typescript
const legacyRoute = zyntra.query({
  path: '/old-route',
  handler: async ({ response }) => {
...
<Callout type="warn" title="Cookie Prefix Enforcement">
  ZyntraJS **automatically enforces** security requirements for `__Secure-` and `__Host-` cookie prefixes according to RFC 6265bis.
</Callout>
...
```typescript
const liveNotifications = zyntra.query({
...
**Publishing Events:**
```typescript
import { SSEProcessor } from '@zyntra-js/core';
...
<Callout type="info" title="SSE Architecture">
  ZyntraJS uses a **centralized SSE endpoint** (`/api/v1/sse/events`) for all real-time streams. Actions return connection information instead of creating individual SSE endpoints.
</Callout>
...
```typescript
const createPost = zyntra.mutation({
  path: '/posts',
  method: 'POST',
...
```typescript
const updatePost = zyntra.mutation({
  path: '/posts/:id' as const,
  method: 'PATCH',
  handler: async ({ request, response, context }) => {
...
<Callout type="success" title="Real-Time Cache Updates">
  When you call `response.revalidate()`, ZyntraJS automatically notifies all connected clients via SSE to refetch invalidated queries!
</Callout>
...
<Callout type="info" title="Discriminated Union">
  The `ZyntraResponse` type is a **discriminated union**: either `data` is present (and `error` is `null`), or `error` is present (and `data` is `null`). Never both!
</Callout>
...
```typescript
const authController = zyntra.controller({
  path: '/auth',
  actions: {
    login: zyntra.mutation({
      path: '/login',
      method: 'POST',
      body: LoginSchema,
      handler: async ({ request, response }) => {
