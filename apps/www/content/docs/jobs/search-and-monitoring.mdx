...
```typescript
// Search all jobs
const allJobs = await zyntra.jobs.search();

// Search jobs in a specific queue
const queueJobs = await zyntra.jobs.search({
...
```typescript
// Find waiting jobs
const waitingJobs = await zyntra.jobs.search({
  filter: {
    status: ['waiting'],
  },
});
...
// Find failed jobs
const failedJobs = await zyntra.jobs.search({
  filter: {
    status: ['failed'],
  },
});
...
const activeJobs = await zyntra.jobs.search({
  filter: {
...
```typescript
// Find specific job
const job = await zyntra.jobs.search({
  filter: {
...
const recentJobs = await zyntra.jobs.search({
  filter: {
    dateRange: {
...
const lastWeekJobs = await zyntra.jobs.search({
  filter: {
    dateRange: {
...
const recentFailures = await zyntra.jobs.search({
  filter: {
    status: ['failed'],
    dateRange: {
...
```typescript
// Get first 50 jobs
const firstPage = await zyntra.jobs.search({
  filter: {
    limit: 50,
    offset: 0,
  },
...
// Get next 50 jobs
const secondPage = await zyntra.jobs.search({
  filter: {
    limit: 50,
    offset: 50,
  },
...
const newestJobs = await zyntra.jobs.search({
  filter: {
...
const priorityJobs = await zyntra.jobs.search({
  filter: {
...
const oldestJobs = await zyntra.jobs.search({
  filter: {
...
```typescript
const results = await zyntra.jobs.search();
...
async function checkQueueHealth() {
  const [waiting, active, completed, failed] = await Promise.all([
    zyntra.jobs.search({ filter: { status: ['waiting'] } }),
    zyntra.jobs.search({ filter: { status: ['active'] } }),
    zyntra.jobs.search({ filter: { status: ['completed'] } }),
    zyntra.jobs.search({ filter: { status: ['failed'] } }),
  ]);
...
async function analyzeJobPerformance() {
  const completedJobs = await zyntra.jobs.search({
    filter: {
      status: ['completed'],
      dateRange: {
...
async function getDashboardMetrics() {
  const [
    ...
    zyntra.jobs.search({ filter: { status: ['waiting'], limit: 100 } }),
    zyntra.jobs.search({ filter: { status: ['active'], limit: 100 } }),
    zyntra.jobs.search({ filter: { status: ['completed'], limit: 100 } }),
    zyntra.jobs.search({ filter: { status: ['failed'], limit: 100 } }),
    zyntra.jobs.search({ filter: { status: ['delayed'], limit: 100 } }),
  ]);
...
async function checkForIssues() {
  ...
  const stalledJobs = await zyntra.jobs.search({
    filter: {
      status: ['stalled'],
    },
  });
...
async function debugJob(jobId: string) {
  const jobs = await zyntra.jobs.search({
    filter: {
      jobId: jobId,
    },
  });
...
```typescript
const stuckJobs = await zyntra.jobs.search({
  filter: {
    status: ['active'],
    dateRange: {
...
```typescript
const recentCompleted = await zyntra.jobs.search({
  filter: {
    status: ['completed'],
    dateRange: {
...
```typescript
const highPriorityWaiting = await zyntra.jobs.search({
  filter: {
    status: ['waiting'],
    orderBy: 'priority:desc',
    limit: 20,
