...
Scheduling allows you to execute jobs at specific times in the future, rather than immediately. ZyntraJS provides flexible scheduling options, from simple delays to complex recurring patterns.
...
```typescript
await zyntra.jobs.analytics.schedule({
  task: 'generateReport',
...
```typescript
await zyntra.jobs.maintenance.schedule({
...
```typescript
await zyntra.jobs.notifications.schedule({
  task: 'sendReminder',
  input: { userId: '123' },
  repeat: {
...
```typescript
await zyntra.jobs.marketing.schedule({
  task: 'sendCampaign',
...
```typescript
await zyntra.jobs.api.schedule({
...
```typescript
await zyntra.jobs.weekly.schedule({
  task: 'weeklySummary',
  input: {},
  repeat: {
...
```typescript
import { SchedulePatterns } from '@zyntra-js/core';

// Run every minute
await zyntra.jobs.monitoring.schedule({
  task: 'checkHealth',
  input: {},
...
// Run every hour
await zyntra.jobs.sync.schedule({
  task: 'syncData',
  input: {},
...
await zyntra.jobs.cleanup.schedule({
  task: 'cleanup',
  input: {},
...
```typescript
await zyntra.jobs.reports.schedule({
  task: 'dailySummary',
  input: {},
  repeat: {
