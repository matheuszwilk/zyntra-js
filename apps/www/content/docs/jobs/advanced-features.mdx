...
Beyond basic job execution, ZyntraJS Jobs provides powerful advanced features for building resilient, observable, and flexible job processing systems.
...
await zyntra.jobs.tasks.schedule({
  task: 'webhookTask',
  input: { data: 'test' },
...
const importantJobs = await zyntra.jobs.search({
  filter: {
