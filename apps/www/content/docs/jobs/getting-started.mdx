...
This guide will walk you through setting up the BullMQ adapter and creating your first background job. By the end, you'll have a working job system integrated with your ZyntraJS application.
...
<Tabs items={['npm', 'pnpm', 'yarn', 'bun']} groupId="package-manager">
  <Tab value="npm">
    ```bash
    npm install @zyntra-js/adapter-bullmq @zyntra-js/adapter-redis bullmq ioredis
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm add @zyntra-js/adapter-bullmq @zyntra-js/adapter-redis bullmq ioredis
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @zyntra-js/adapter-bullmq @zyntra-js/adapter-redis bullmq ioredis
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @zyntra-js/adapter-bullmq @zyntra-js/adapter-redis bullmq ioredis
    ```
  </Tab>
</Tabs>
...
## Step 5: Register Jobs with Zyntra

Register your jobs with the Zyntra builder:
...
export const signup = zyntra.mutation({
  path: '/signup',
  method: 'POST',
  body: z.object({
    email: z.string().email(),
