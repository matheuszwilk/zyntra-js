 ...
 *
 * @see https://github.com/matheuszwilk/zyntra-js/tree/main/packages/adapter-mcp-server
 */
const { handler, auth } = ZyntraMcpServer
  ...
  .withInstructions('This is a demo Zyntra.js MCP server. Use the available tools to interact with the API.')
  ...
  .addPrompt({
    name: 'exploreAPI',
    description: 'Get guidance on exploring the Zyntra.js API',
    args: {},
    handler: async (args, context) => {
      // context is automatically typed from the router!
      return {
