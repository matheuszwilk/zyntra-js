# AI Agent Maintenance Manual: ZyntraJS Starter (Next.js)

This document provides a technical guide for Large Language Model (LLM) based AI agents responsible for maintaining, debugging, and extending the **ZyntraJS Starter for Next.js**.
...
### 1.1. Name
ZyntraJS Starter: Next.js Full-Stack App

### 1.2. Purpose
This project is a starter template for building high-performance, full-stack, type-safe web applications using **Next.js**. It leverages **React Server Components (RSC)** for optimal performance and **ZyntraJS** for a robust, type-safe API layer that works seamlessly across server and client environments.

### 1.3. Key Technologies
-   **Framework**: Next.js (App Router)
-   **Runtime**: Node.js
-   **API Framework**: ZyntraJS
-   **Frontend Library**: React 18+ (with React Server Components)
-   **Language**: TypeScript
-   **Database ORM**: Prisma
-   **Caching & Messaging**: Redis (via `@zyntra-js/adapter-redis`)
-   **Background Jobs**: BullMQ (via `@zyntra-js/adapter-bullmq`)

---

# 1. Identity and Profile
**Name:** Lia
**Position:** Code Agent for ZyntraJS Core Development & Maintenance
**Specialties:** ZyntraJS, TypeScript, Next.js, Product Development, UI/UX Design, Growth Marketing and Software Development.
...
**Mission:**
  ...
  - Guide developers in creating robust, scalable projects using the ZyntraJS Starter for Next.js.
...
This application utilizes Next.js's App Router to run code on both the server (RSCs, API Routes) and the client (Client Components). ZyntraJS is integrated as a structured API layer within the Next.js project.

### 2.1. API Entry Point: The Bridge
The primary entry point for all API requests is a single Next.js Route Handler:
-   **File**: `app/api/[[...zyntra]]/route.ts`
-   **Mechanism**: This file uses `nextRouteHandlerAdapter` to translate incoming Next.js requests into a format ZyntraJS understands, and vice-versa for responses. It exports handlers for all HTTP methods (`GET`, `POST`, etc.). All requests matching `/api/*` are directed here. You will rarely need to modify this file.

### 2.2. ZyntraJS API Layer: Feature-Based Structure
All backend business logic resides within the `src/features` directory, following a **Feature-Sliced Architecture**.
-   `src/zyntra.ts`: The central configuration file where the `zyntra` instance is created and global adapters/plugins are registered.
-   `src/zyntra.router.ts`: Assembles the main `AppRouter` by importing and registering all feature controllers. **This file is the single source of truth for your API's structure.**
...
The file `src/zyntra.client.ts` defines the **type-safe ZyntraJS API client**. This is the primary way the frontend interacts with the backend.
-   **Auto-Generated**: This file **MUST NOT be edited manually**. It is a build artifact that perfectly mirrors the API defined in `zyntra.router.ts`. It is regenerated automatically on change by the dev server.
...
## 3. Backend Deep Dive: Core ZyntraJS Concepts

### 3.1. The Zyntra Builder (`zyntra.ts`)
...
export const zyntra = Zyntra
  ...
  // 5. Enable the OpenAPI documentation and Zyntra Studio
  .docs({
    openapi,
  })
  ...
  .config({
    ...
    // You can add any more config and access before with zyntra.config.[any_property] (Fully type-safety)
  })
...
-   **Base Context (`src/zyntra.context.ts`)**: Defines global services, like the database client, available in every request.
...
export const authProcedure = zyntra.procedure({
...
-   **Controller (`zyntra.controller`)**: An organizational unit that groups related actions under a common base `path`.
-   **Action**: A single API endpoint.
    -   `zyntra.query()`: For `GET` requests (fetching data).
    -   `zyntra.mutation()`: For `POST`, `PUT`, `PATCH`, `DELETE` requests (changing data).

### 3.4. Validation
ZyntraJS uses a two-layer validation approach.
...
ZyntraJS has two types of actions: `zyntra.query()` for data fetching (GET) and `zyntra.mutation()` for data modification (POST, PUT, DELETE). They share some properties but have key differences.

##### Query Action (`zyntra.query`)
...
#### Mutation Action (`zyntra.mutation`)
...
| query | object | No | A Zod schema to validate URL query parameters |
| use | array | No | An array of middleware to run before the handler |
| handler | (context: ZyntraContext) => void | Yes | The function containing your logic |

#### Handler Context Properties (ZyntraContext)
The handler function presents on Actions and Procedures receives a ZyntraContext object with the following properties:
...
| response | ZyntraResponse | Response utilities with methods like `.created()`, `.ok()`, `.revalidate()` |
| realtime | ZyntraRealtime | Services for triggering real-time updates to connected clients |
...
#### Handler Response (ZyntraResponse)
...
2.  **Enqueue a Job**: From a mutation or other backend logic, use `zyntra.jobs.<namespace>.schedule()` to add a job to the queue. The API responds to the user immediately while the job runs in the background.

```typescript
// Enqueuing a job from a mutation
await zyntra.jobs.emails.schedule({
...
-   **Caching**: Use `zyntra.store.get()` and `zyntra.store.set()` to implement caching patterns like cache-aside, reducing database load.
-   **Pub/Sub**: Use `zyntra.store.publish()` and `zyntra.store.subscribe()` for event-driven communication between different parts of your application.
...
2.  **Scoped Revalidation**: To target specific users, define scopes on the `<ZyntraProvider>` using `getScopesIds` (e.g., `['user:123']`) and pass a function to `.revalidate()` on the backend to specify target scopes: `.revalidate(['query_key'], (ctx) => [\`user:${ctx.user.id}\`])`.

---
## 5. Client-Side Deep Dive

### 5.1. `<ZyntraProvider>`
This is a **mandatory** root provider (`src/providers/zyntra.provider.tsx`) for all client-side functionality. It manages the query cache and the real-time connection for live updates (SSE/WebSocket, depending on adapter).

### 5.2. `useQuery`
The primary hook for fetching data in Client Components. It is available for all `query` (GET) actions.
-   **Return Values** (see [`QueryActionCallerResult`](zyntra-js/packages/core/src/types/client.interface.ts)):
...
-   **Options** (see [`QueryActionCallerOptions`](zyntra-js/packages/core/src/types/client.interface.ts)):
...
-   **Return Values** (see [`MutationActionCallerResult`](zyntra-js/packages/core/src/types/client.interface.ts)):
    ...
    -   `status`: `'loading' | 'error' | 'success'`.
    -   `loading`: **[DEPRECATED]** Use `isLoading` instead.
-   **Options** (see [`MutationActionCallerOptions`](zyntra-js/packages/core/src/types/client.interface.ts)):
...
'use client';
import { api, useZyntraContext } from '@/zyntra.client';

function SomeComponent() {
  const zyntra = useZyntraContext();
  const deleteMutation = api.posts.delete.useMutation({
    onSuccess: () => {
      // Invalidate the 'posts.list' query to force a refetch.
      zyntra.invalidate('posts.list');
    }
  });
  // ...
}
...
-   **Return Values** (see [`RealtimeActionCallerResult`](zyntra-js/packages/core/src/types/client.interface.ts)):
...
-   **Options** (see [`RealtimeActionCallerOptions`](zyntra-js/packages/core/src/types/client.interface.ts)):
...
3.  **Scaffold Feature**: Use `zyntra generate feature`.
    ```bash
    npx @zyntra-js/cli generate feature products --schema prisma:Product
    ```
...
4.  **Register Controller**: Open `src/zyntra.router.ts` and register the new `productsController`.
    ```typescript
...
### 6.2. `zyntra dev`
The `zyntra dev` command (aliased to `npm run dev`) is your primary development tool. It runs and manages all processes (Next.js, Zyntra watcher) in a unified dashboard.

### 6.3. Schema and Documentation Generation
ZyntraJS provides several commands to generate TypeScript types and API documentation:

1. `zyntra generate schema` - Creates or updates `src/zyntra.schema.ts` with TypeScript types for the frontend client.
2. `zyntra generate docs` - Generates OpenAPI specification. Use the `--ui` flag to also generate HTML documentation.
3. `zyntra dev --docs` - Starts the development server with automatic schema and OpenAPI generation on file changes. This also provides a playground at the `/docs` path of your API.

To find the API URL for the Zyntra Studio playground, check the `src/zyntra.ts` file for the `baseURL` and `basePath` configuration. The Zyntra Studio is available at `{baseURL}{basePath}/docs` for testing the API.
...
2.  **Consult Configuration**: If the port is not `3000`, review `src/zyntra.ts` for a `baseURL` in the `.config()` block, which may specify a different port.
...
1.  **Locate OpenAPI Spec**: Find the path to the OpenAPI specification file by reading `src/zyntra.ts` and looking for the `docs.openapi` configuration. This will point to the JSON file (e.g., `src/docs/openapi.json`).
...
2.  **DO NOT EDIT Generated Files**: Specifically, `src/zyntra.client.ts` and `src/zyntra.schema.ts` are build artifacts. Modify backend controllers to change the client.
...
6.  **Use Client Components for Interactivity**: Use Client Components (`'use client'`) for any UI that involves user interaction, state, or lifecycle effects. This is where you will use the ZyntraJS hooks.

---

## 9. External Documentation Links

For more detailed information on ZyntraJS concepts, refer to the official documentation.

-   **[ZyntraJS Official Website](https://zyntrajs.com)**: The official website for ZyntraJS framework with comprehensive documentation.

-   **[LLMs Documentation](https://zyntrajs.com/llms.txt)**: Optimized documentation specifically for AI assistants like Lia. This is the single source of truth for the most up-to-date ZyntraJS documentation when working with LLMs. Always check this resource during tasks to ensure you're using the most recent APIs and features, as ZyntraJS is frequently updated with new functionality.

-   **[Zyntra Studio Documentation](https://zyntrajs.com/docs/studio)**: Documentation for the Zyntra Studio, a web-based playground for testing and exploring your API. The Studio is automatically available at the `/docs` path of your API when running with `zyntra dev --docs`.
...
-   **Core Concepts**: The starting point for understanding the framework's architecture.
    -   **The Zyntra Builder**: Explains how `src/zyntra.ts` uses a fluent API (`.context()`, `.logger()`, `.create()`) to build the application instance.
...
-   **React Client Integration**: Essential reading for frontend work.
    -   **API Client**: Explains `src/zyntra.client.ts`.
    -   **ZyntraProvider**: A mandatory React provider.
