...
Welcome to the ZyntraJS starter for building high-performance, type-safe REST APIs with **Deno**. This template provides a robust and scalable foundation for creating modern back-end services.
...
2.  **Configure Environment Variables**
    Create a `.env` file in the root of this starter (`zyntra-js/apps/starter-deno-rest-api/.env`) and add your database and Redis connection URLs:
...
The `src/index.ts` file is the main entry point. It uses `serve` from `deno.land/std/http` to handle incoming requests. A simple path-based router checks if the request URL starts with `/api/v1/` and, if so, forwards it to the ZyntraJS API router handler.
...
-   **Cache Demonstration**
    ...
    curl http://localhost:8000/api/v1/user/cache/my-key
...
-   **Schedule a Background Job**
    ...
    curl -X POST -H "Content-Type: application/json" \
      ...
      http://localhost:8000/api/v1/user/schedule-job
    ```

-   **List Jobs**
    ```bash
    curl http://localhost:8000/api/v1/user/jobs
    ```
...
-   **[Store (Redis)](https://zyntrajs.com/docs/advanced-features/store)**
-   **[Queues (BullMQ)](https://zyntrajs.com/docs/advanced-features/queues)**
