# Code Agent Instructions: ZyntraJS Starter (Deno REST API)
...
### 1.1. Name
ZyntraJS Starter: Deno REST API

### 1.2. Purpose
This project is a high-performance starter template for building **type-safe REST APIs**. It uses **Deno** as the runtime and **ZyntraJS** as the core framework. It is designed for back-end services that require scalability, maintainability, and strong type guarantees in a modern, secure runtime environment.
...
-   **Routing**: A simple manual routing check is performed. If an incoming request `URL.pathname` starts with `/api/v1/`, the request is forwarded to the ZyntraJS handler. Otherwise, a `404 Not Found` response is returned.
...
### 2.2. ZyntraJS API Layer
The back-end logic is powered by ZyntraJS and follows a structured, feature-based pattern.
-   `src/zyntra.ts`: This is the **central configuration file**. It creates the main `zyntra` instance and registers all adapters (Redis store, BullMQ jobs, logger). You will modify this file only to add new global adapters or plugins.
-   `src/zyntra.router.ts`: This file **assembles the main API router**. It imports all feature controllers and registers them under specific path prefixes (e.g., `user` maps to `/api/v1/user`).
...
The file `src/zyntra.client.ts` is an auto-generated, type-safe client.
...
3.  **Define Controller and Action**:
    ...
    export const productsController = zyntra.controller({
      name: 'Products',
      path: '/products',
      actions: {
        list: zyntra.query({
    ...
    ```
4.  **Register Controller**: Open `src/zyntra.router.ts` and add the new controller.
    ```typescript
...
>
> Instead of creating these files manually, you can use the `zyntra generate` command to build a complete feature slice from your Prisma schema in one step. This is the recommended approach.
>
> ```bash
...
> ```
> This saves time and ensures consistency across your application. After running the command, you just need to register the new controller in `src/zyntra.router.ts`.
...
1.  **Define Job**: Open `src/services/jobs.ts` and add the job definition inside the `jobs.router` call within `REGISTERED_JOBS`.
    ...
    ```
2.  **Enqueue Job**: From a relevant API action (e.g., a `products.create` mutation), enqueue the job using the `zyntra.jobs` instance.
    ```typescript
    // In a controller action handler
    const jobInfo = await zyntra.jobs.system.enqueue({
      task: 'processImage',
      input: {
    ...
    zyntra.logger.info('Scheduled image processing job', { jobId: jobInfo.id });
    ```
...
The type-safe client (`src/zyntra.client.ts`) and schema (`src/zyntra.schema.ts`) are the bridge between your back-end and any potential front-end consumer. They are generated by introspecting your `AppRouter`.

-   **Manual Generation**: You can run the command `deno task build:client` (which aliases `zyntra generate schema`) to perform a one-time generation. This is useful in CI/CD pipelines or when you need to provide the client artifacts to another team.
-   **Automatic Generation**: The development server (`deno task dev`) runs `zyntra dev`, which automatically watches for changes in your controllers and regenerates the client instantly.
...
3.  **DO NOT EDIT Build Artifacts**: The files `src/zyntra.client.ts` and `src/zyntra.schema.ts` are automatically generated. Do not edit them manually.
...
For more detailed information on ZyntraJS concepts, refer to the official documentation wiki.

-   **[Core Concepts](https://zyntrajs.com/docs/core-concepts)**: Understand Actions, Controllers, Context, and the builder pattern.
-   **[Store (Redis)](https://zyntrajs.com/docs/advanced-features/store)**: Learn about caching (`get`, `set`) and Pub/Sub (`publish`, `subscribe`).
-   **[Queues (BullMQ)](https://zyntrajs.com/docs/advanced-features/queues)**: Learn how to define, schedule, and manage background jobs.
-   **[API Client](https://zyntrajs.com/docs/client-side/api-client)**: Understand how the type-safe client is generated for consumers.
