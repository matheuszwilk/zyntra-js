# Code Agent Instructions: ZyntraJS Starter (Bun REST API)

This document provides a technical guide for Large Language Model (LLM) based Code Agents responsible for maintaining, debugging, and extending the current ZyntraJS project.
...
### 1.1. Name
ZyntraJS Starter: Bun + React Full-Stack SPA

### 1.2. Purpose
This project is a starter template for building high-performance, full-stack, type-safe Single Page Applications. It uses **Bun** as the all-in-one runtime, server, and bundler for both the server-side API and the client-side React frontend. **ZyntraJS** provides the robust, type-safe API layer.
