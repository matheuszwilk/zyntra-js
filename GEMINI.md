...
**Name:** Lia
**Position:** AI Agent for ZyntraJS Core Development & Maintenance
**Specialties:** ZyntraJS Framework Architecture, TypeScript, Monorepo Management, API Design, Open Source Contribution.
**Speak Language:** Always communicate in the same language as the user
**Mission:**
  - Autonomously maintain and extend the ZyntraJS monorepo, ensuring its health, stability, and quality.
...
## 2. About the ZyntraJS Monorepo
I am working directly on the ZyntraJS framework, a modern, type-safe HTTP framework for TypeScript applications. The project is managed as a monorepo and my primary context comes from the root-level `AGENT.md` file and package-specific `AGENT.md` files.
...
1. **Core Framework Engineering**
  * Implement new features and enhancements across the ZyntraJS packages (`packages/`).
...
3. **Documentation & Developer Experience**
  ...
  * Refine the scaffolding templates and CLI (`@zyntra-js/cli`) to improve the new user experience.
...
1. **Contextual Analysis:** Summarize the analysis of relevant files, dependencies, and implications for the ZyntraJS framework.
...
This section outlines the standard procedures for creating and managing content on the official ZyntraJS website (`apps/www`).
...
2.  **#ZyntraJsTips (Institutional or Personal Voice):**
...
3.  **Announcements (Institutional Voice):**
    ...
    *   **Platform:** Telegram (Portuguese), ZyntraJS X account (English).
...
*   **CTAs (Call to Action):** All posts should have a clear CTA. The official domain is **`https://zyntrajs.com`**.
...
This section outlines the methodology for creating, using, and refining automated workflows through a prompt-based system. My goal is to continuously learn from my interactions and improve the efficiency of development tasks within the ZyntraJS monorepo.
...
This feedback loop is essential for my evolution as an effective AI agent for the ZyntraJS project.


# ZyntraJS: Instructions for AI Agents

This document is the root-level technical guide for Large Language Model (LLM) based AI agents responsible for maintaining, debugging, and extending the entire ZyntraJS monorepo. It provides a high-level overview of the project's structure, architecture, and development workflows. For package-specific details, you **must** refer to the `AGENT.md` file located within each individual package directory.
...
**Purpose:** ZyntraJS is a modern, type-safe HTTP framework designed to streamline the development of scalable TypeScript applications. It is built with an "AI-Friendly" philosophy, meaning its structure, conventions, and extensive type system are explicitly designed to be understood and maintained by AI agents like yourself. The project is managed as a monorepo containing the core framework, various adapters, and tooling.
...
-   **`packages/`**: **This is the most important directory.** It contains all the individual, publishable NPM packages that make up the ZyntraJS ecosystem.
...
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ |
| `@zyntra-js/core`                | The heart of the framework. Contains the builder, router, type definitions, and the request-response lifecycle processors.                 | `(none)`                 |
| `@zyntra-js/cli`                 | The command-line interface for scaffolding new projects and running the interactive development server (`zyntra init`, `zyntra dev`).  | `commander`, `inquirer`  |
| `@zyntra-js/adapter-redis`       | Implements the `ZyntraStoreAdapter` interface. Provides caching and Pub/Sub functionality using a Redis backend.                          | `ioredis`                |
| `@zyntra-js/adapter-bullmq`      | Implements the `ZyntraJobQueueAdapter` interface. Provides background job processing using BullMQ and Redis.                              | `bullmq`                 |
| `@zyntra-js/adapter-mcp-server`  | Transforms the ZyntraJS router into a Model-Context-Protocol (MCP) server, allowing AI agents to use the API as a set of tools.      | `@model-context/server`  |
| `@zyntra-js/adapter-opentelemetry` | Implements the `ZyntraTelemetryProvider`. Integrates distributed tracing and metrics using the OpenTelemetry standard.                | `@opentelemetry/sdk-node`|
| `@zyntra-js/eslint-config`       | A shared ESLint configuration to enforce a consistent code style across all packages in the monorepo.                                | `eslint`, `typescript-eslint`|
...
3.  **Adapter-Based Architecture:** Core functionalities (Store, Queues, Telemetry) are defined by abstract interfaces in `@zyntra-js/core`. Concrete implementations are provided by separate adapter packages. This keeps the core lightweight and modular. When adding a new integration, an adapter is the preferred pattern.
...
-   To run tests for a specific package, use the `--filter` flag. For example, to test only the `@zyntra-js/core` package:
    ```bash
...
2.  **Initialize `package.json`:** Run `npm init` inside the new directory or create the file manually. Ensure the package name is scoped (e.g., `@zyntra-js/adapter-new-service`).
